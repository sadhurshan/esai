...
{
    private const PREVIOUS_TYPES = ['rfq_draft', 'supplier_message', 'maintenance_checklist', 'inventory_whatif', 'invoice_draft'];
    private const UPDATED_TYPES = ['rfq_draft', 'supplier_message', 'maintenance_checklist', 'inventory_whatif', 'invoice_draft', 'approve_invoice'];
    ...
    private function rebuildSqliteTable(array $allowedValues): void
    {
        ...
        $tempTable = 'ai_action_drafts_tmp_v2';

        Schema::create($tempTable, function (Blueprint $table) use ($allowedValues): void {
            $table->id();
            ...
            $table->index(['company_id', 'status', 'created_at'], 'ai_action_drafts_tmp_v2_company_status_created_at_index');
            $table->index(['entity_type', 'entity_id'], 'ai_action_drafts_tmp_v2_entity_type_entity_id_index');
        });
        ...
        DB::statement(sprintf(
            'INSERT INTO %s (%s) SELECT %s FROM ai_action_drafts',
            $tempTable,
            $columnList,
            $columnList
        ));
        ...
        Schema::rename($tempTable, 'ai_action_drafts');
