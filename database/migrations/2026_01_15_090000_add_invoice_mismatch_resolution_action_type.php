...
return new class extends Migration
{
    ...
    private function rebuildSqliteTable(array $allowedValues): void
    {
        ...
        Schema::create('ai_action_drafts_tmp_v4', function (Blueprint $table) use ($allowedValues): void {
            $table->id();
            ...
            $table->index(['company_id', 'status', 'created_at'], 'ai_action_drafts_tmp_v4_company_status_created_at_idx');
            $table->index(['entity_type', 'entity_id'], 'ai_action_drafts_tmp_v4_entity_type_entity_id_idx');
        });
        ...
        DB::statement(sprintf(
            'INSERT INTO ai_action_drafts_tmp_v4 (%1$s) SELECT %1$s FROM ai_action_drafts',
            $columnList
        ));

        Schema::drop('ai_action_drafts');
        Schema::rename('ai_action_drafts_tmp_v4', 'ai_action_drafts');
