        ...
        <Card data-testid={`api-key-${apiKey.id}`}>
            ...
            <CardContent className="space-y-4">
                <dl className="grid gap-4 text-sm sm:grid-cols-2">
                    ...
                    <Metadata
                        ...
                        value={
                            apiKey.ownerUserId
                                ? `User #${apiKey.ownerUserId}`
                                : '—'
                        }
                    />
                    <Metadata
                        label="Last used"
                        value={lastUsedLabel ?? 'Never'}
                    />
                    <Metadata
                        label="Expires"
                        value={expiresLabel ?? 'Never'}
                        warning={Boolean(apiKey.expiresAt && !apiKey.active)}
                    />
                </dl>
            ...
            <CardFooter className="justify-between border-t pt-4">
                <div className="flex items-center gap-2 text-xs text-muted-foreground">
                    ...
                    Treat API keys like passwords. Revoke unused credentials
                    immediately.
                </div>
