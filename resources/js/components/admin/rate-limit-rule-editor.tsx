...
import type { RateLimitRuleInput, RateLimitRuleItem, SyncRateLimitPayload } from '@/types/admin';
...
export function RateLimitRuleEditor({ rules, isLoading = false, isSaving = false, onSave }: RateLimitRuleEditorProps) {
    const [drafts, setDrafts] = useState<RuleDraft[]>(() => rules?.map((rule) => mapRuleToDraft(rule)) ?? []);

    const hasChanges = useMemo(() => computeDirtyState(drafts, rules ?? []), [drafts, rules]);
    const errorMap = useMemo(() => new Map(drafts.map((draft) => [draft.clientId, validateDraft(draft)])), [drafts]);
    const hasValidationIssues = useMemo(() => Array.from(errorMap.values()).some(Boolean), [errorMap]);

    const pendingDeletions = drafts.filter((draft) => draft.markForDeletion);

    const handleFieldChange = (clientId: string, field: keyof RateLimitRuleInput, value: string | number | boolean) => {
        setDrafts((prev) =>
            prev.map((draft) =>
                draft.clientId === clientId
                    ? {
                          ...draft,
                          [field]: typeof value === 'string' && field !== 'scope' ? Number(value) : value,
                      }
                    : draft,
            ),
        );
    ...
    return (
        <div className="space-y-6">
            <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                <div>
                    <p className="text-base font-semibold text-foreground">Rate limit rules</p>
                    <p className="text-sm text-muted-foreground">
                        Define throttles per integration scope. Tenants share scopes such as `public_api`, `supplier_api`, and `web_app`.
                    </p>
                </div>
                <div className="flex gap-2">
                    <Button type="button" variant="outline" onClick={handleReset} disabled={!hasChanges || isSaving}>
                        <RefreshCcw className="mr-2 h-4 w-4" aria-hidden /> Reset
                    </Button>
                    <Button type="button" onClick={handleAddRule} disabled={isSaving}>
            ...
            {drafts.length === 0 ? (
                ...
                <div className="space-y-4">
                    {drafts.map((draft) => {
                        ...
                        const example = LIMIT_EXAMPLES[draft.scope] ?? 'Custom scope';
                        const preview = buildPreviewLabel(draft);
                        const isExisting = Boolean(draft.id);
                        const markedForDeletion = Boolean(draft.markForDeletion);
                        return (
                            <Card
                                key={draft.clientId}
                                className={cn(
                                    'border border-muted/60',
                                    !markedForDeletion && !draft.active && 'opacity-90',
                                    markedForDeletion && 'border-destructive/40 bg-destructive/5',
                                )}
                            ...
                            >
                                <CardHeader className="gap-1">
                                    <CardTitle className="flex flex-wrap items-center gap-2 text-lg">
                                        <Input
                                            value={draft.scope}
                                            onChange={(event) => handleFieldChange(draft.clientId, 'scope', event.target.value)}
                                            placeholder="Scope key (e.g. public_api)"
                                            disabled={isExisting}
                                            className="max-w-sm"
                                        />
                                        <Badge variant={markedForDeletion ? 'destructive' : draft.active ? 'default' : 'outline'}>
                                            {markedForDeletion ? 'Pending deletion' : draft.active ? 'Active' : 'Disabled'}
                                        </Badge>
                                    </CardTitle>
                                    <CardDescription>{markedForDeletion ? 'Rule will be removed on save.' : example}</CardDescription>
                                </CardHeader>
                                <CardContent className="space-y-4">
                                    <div className="grid gap-4 md:grid-cols-3">
                                        <div className="space-y-2">
                                            <label className="text-xs font-medium uppercase text-muted-foreground">Window (seconds)</label>
                                            <Input
                                                type="number"
                                                min={5}
                                                step={5}
                                                value={draft.windowSeconds ?? ''}
                                                onChange={(event) => handleFieldChange(draft.clientId, 'windowSeconds', event.target.value)}
                                                disabled={markedForDeletion}
                                            />
                                        </div>
                                        <div className="space-y-2">
                                            <label className="text-xs font-medium uppercase text-muted-foreground">Max requests</label>
                                            <Input
                                                type="number"
                                                min={1}
                                                step={1}
                                                value={draft.maxRequests ?? ''}
                                                onChange={(event) => handleFieldChange(draft.clientId, 'maxRequests', event.target.value)}
                                                disabled={markedForDeletion}
                                            />
                                        </div>
                                        <div className="space-y-2">
                                            ...
                                            <div className="flex items-center gap-2 rounded-lg border bg-muted/40 px-3 py-2 text-sm">
                                                <Checkbox
                                                    ...
                                                    onCheckedChange={(state) => handleToggleActive(draft.clientId, Boolean(state))}
                                                    disabled={markedForDeletion}
                                                />
                                ...
                                <CardContent className="flex flex-wrap justify-between gap-3 border-t pt-4">
                                    ...
                                    <div className="flex gap-2">
                                        <Button type="button" variant="ghost" onClick={() => handleRemove(draft.clientId)}>
                                            <Trash2 className="mr-2 h-4 w-4" aria-hidden />
                                            {markedForDeletion ? 'Undo delete' : isExisting ? 'Delete' : 'Remove'}
                                        </Button>
                                    </div>
                                </CardContent>
                            </Card>
            ...
            {pendingDeletions.length ? (
                <div className="rounded-lg border border-destructive/40 bg-destructive/5 p-3 text-sm text-destructive">
                    {pendingDeletions.length} rule{pendingDeletions.length === 1 ? '' : 's'} marked for deletion.
                </div>
            ) : null}
            ...
            <div className="flex flex-wrap justify-end gap-3 border-t pt-4">
                {hasValidationIssues ? (
                    <p className="text-sm text-destructive flex items-center gap-2">
                        <AlertTriangle className="h-4 w-4" aria-hidden /> Resolve validation issues before saving.
                    </p>
                ) : null}
                <Button type="button" onClick={handleSubmit} disabled={!hasChanges || hasValidationIssues || isSaving}>
...
function buildPayload(drafts: RuleDraft[]): SyncRateLimitPayload {
    ...
    const removals = drafts.filter((draft) => draft.markForDeletion && draft.id).map((draft) => draft.id as number);
...
function computeDirtyState(drafts: RuleDraft[], baseline: RateLimitRuleItem[]): boolean {
    const baselineMap = new Map(baseline.map((rule) => [rule.id, rule]));

    if (drafts.length !== baseline.length) {
        const visibleCount = drafts.filter((draft) => !draft.markForDeletion).length;
...
function validateDraft(draft: RuleDraft): string | null {
    ...
    if (!Number.isFinite(draft.windowSeconds) || (draft.windowSeconds ?? 0) <= 0) {
