...
export function WebhookEndpointEditor({
    ...
    const safeRetryBackoff: 'exponential' | 'linear' =
        defaultRetryPolicy.backoff === 'linear' ? 'linear' : 'exponential';
    ...
    const retryBackoffValue =
        useWatch({ control: form.control, name: 'retryBackoff' }) ??
        safeRetryBackoff;
        ...
        <form className="space-y-6" onSubmit={handleSubmit}>
            <section className="space-y-4 rounded-xl border bg-card p-4">
                <div className="flex flex-col gap-1">
                    ...
                    <p className="text-sm text-muted-foreground">
                        Choose which tenant and URL should receive events.
                    </p>
                </div>
                    ...
                    <div className="space-y-2">
                        ...
                        {form.formState.errors.url ? (
                            ...
                            <p className="text-xs text-muted-foreground">
                                Must accept HTTPS POST requests.
                            </p>
                        )}
                    </div>
                </div>
                <div className="flex flex-col gap-2">
                    ...
                    <p className="text-xs text-muted-foreground">
                        Inactive endpoints will queue events but skip delivery
                        attempts.
                    </p>
                </div>
            </section>
            ...
            <section className="space-y-4 rounded-xl border bg-card p-4">
                <div className="flex flex-col gap-1">
                    ...
                    <p className="text-sm text-muted-foreground">
                        We include this secret in the `X-Webhook-Signature`
                        header.
                    </p>
                </div>
                <div className="grid gap-4 md:grid-cols-[2fr_1fr]">
                    <div className="space-y-2">
                        ...
                        {form.formState.errors.secret ? (
                            ...
                            <p className="text-xs text-muted-foreground">
                                Current secret ends with{' '}
                                {subscription.secretHint}.
                            </p>
                        ) : (
                            <p className="text-xs text-muted-foreground">
                                Minimum 16 characters. Leave empty to keep the
                                current value.
                            </p>
                        )}
                    </div>
            ...
            <section className="space-y-4 rounded-xl border bg-card p-4">
                <div className="flex flex-col gap-1">
                    <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                        <div>
                            ...
                            <p className="text-sm text-muted-foreground">
                                Select which notifications this endpoint should
                                receive.
                            </p>
                        </div>
                ...
                <div className="grid gap-3 md:grid-cols-2">
                    {availableEvents.map((event) => {
                        const checked =
                            selectedEvents?.includes(event.value) ?? false;
                        return (
                            <label
                                key={event.value}
                                className={cn(
            ...
            <section className="space-y-4 rounded-xl border bg-card p-4">
                <div className="flex flex-col gap-1">
                    ...
                    <p className="text-sm text-muted-foreground">
                        Control how often we attempt deliveries before
                        dead-lettering.
                    </p>
                </div>
                <div className="grid gap-4 md:grid-cols-3">
                    <div className="space-y-2">
                        ...
                        {form.formState.errors.retryMax ? (
                            <p className="text-sm text-destructive">
                                {form.formState.errors.retryMax.message}
                            </p>
                        ) : (
                            <p className="text-xs text-muted-foreground">
                    ...
                    <div className="space-y-2">
                        <Label htmlFor="webhook-retry-base">
                            Base delay (sec)
                        </Label>
                        <Input
                        ...
                        {form.formState.errors.retryBaseSeconds ? (
                            <p className="text-sm text-destructive">
                                {form.formState.errors.retryBaseSeconds.message}
                            </p>
                        ) : (
                            <p className="text-xs text-muted-foreground">
                                First retry waits this long before applying
                                backoff.
                            </p>
                        )}
                    </div>
...
function extractRetryPolicy(
    ...
    if (
        ...
        typeof subscription.retryPolicy !== 'object'
    ) {
        return fallback;
    }
    ...
    return {
        ...
        baseSeconds:
            typeof policy.base_sec === 'number' ? policy.base_sec : undefined,
    };
}
