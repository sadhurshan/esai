...
export function DocumentNumberPreview({
    docType,
    label,
    hint = 'Final number is assigned when you save.',
    className,
    ...
    if (numberingQuery.isLoading) {
        return (
            <Skeleton
                className={cn('h-20 w-full max-w-sm rounded-lg', className)}
            />
        );
    }
    ...
    if (numberingQuery.isError) {
        return (
            <div
                className={cn(
                    'rounded-lg border border-dashed border-border/70 bg-muted/40 px-4 py-3 text-sm',
                    className,
                )}
            >
    ...
    return (
        <div
            className={cn(
                'rounded-lg border border-dashed border-border/70 bg-muted/20 px-4 py-3 text-sm',
                className,
            )}
        >
            <p className="text-xs tracking-wide text-muted-foreground uppercase">
