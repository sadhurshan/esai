...
export function LocationSelect({
    ...
    return (
        <Select
            ...
            disabled={disabled}
        >
            <SelectTrigger id={id} className={cn('h-9 justify-start gap-2', className)}>
            ...
            <SelectContent>
                ...
                {options.length === 0 ? (
                    ...
                    options.map((option) => (
                        <SelectItem key={option.id} value={option.id} className="flex flex-col gap-0.5">
                            <span className="text-sm font-medium">{option.name}</span>
                            {option.siteName ? (
                                <span className="text-xs text-muted-foreground">{option.siteName}</span>
                            ) : null}
                        </SelectItem>
                    ))
                )}
