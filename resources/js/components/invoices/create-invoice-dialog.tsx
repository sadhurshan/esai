...
const createInvoiceFormSchema = z
    .object({
        ...
        lines: z.array(createInvoiceLineSchema).min(1, 'At least one PO line is required.'),
    })
...
export interface CreateInvoiceDialogProps {
    ...
    purchaseOrder: Pick<PurchaseOrderDetail, 'id' | 'poNumber' | 'currency' | 'supplierName' | 'supplierId' | 'lines'>;
...
export function CreateInvoiceDialog({
    ...
    const submitDisabled =
        isSubmitting || lineValues.length === 0 || lineValues.every((line) => line.remainingQuantity <= 0);

    const handleSubmit = form.handleSubmit((values) => {
        const filteredLines = values.lines.filter((line) => line.qtyInvoiced > 0);
    ...
    return (
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent className="max-w-3xl">
                <DialogHeader>
                    <DialogTitle>Create invoice from PO #{purchaseOrder.poNumber}</DialogTitle>
                    <DialogDescription>
                ...
                <form className="space-y-6" onSubmit={handleSubmit}>
                    <div className="grid gap-4 md:grid-cols-2">
                        ...
                        <div className="space-y-2">
                            ...
                            <Input id="invoice-date" type="date" disabled={isSubmitting} {...form.register('invoiceDate')} />
                    ...
                    <div className="space-y-3">
                        <div className="flex items-center justify-between">
                            ...
                            <p className="text-xs text-muted-foreground">Set quantities up to the remaining amount.</p>
                        </div>
                            ...
                            <table className="w-full text-sm">
                                <thead className="bg-muted/40 text-xs uppercase tracking-wide text-muted-foreground">
                                    <tr>
                                        ...
                                        <th className="px-3 py-2 text-left">Qty to invoice</th>
                                        <th className="px-3 py-2 text-left">Unit price ({purchaseOrder.currency})</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    {lineValues.length === 0 ? (
                                        ...
                                        lineValues.map((line, index) => (
                                            <tr key={`${line.poLineId}-${index}`} className="border-t border-border/60">
                                                <td className="px-3 py-3 align-top">
                                                    <div className="font-semibold text-foreground">Line {line.lineNo}</div>
                                                ...
                                                <td className="px-3 py-3">
                                                    <Input
                                                        ...
                                                        step="0.01"
                                                        min="0"
                                                        disabled={isSubmitting || line.remainingQuantity <= 0}
                                                        {...form.register(`lines.${index}.qtyInvoiced` as const)}
                                                    />
                                                    {Array.isArray(form.formState.errors.lines) &&
                                                    form.formState.errors.lines[index]?.qtyInvoiced ? (
                                                        <p className="pt-1 text-xs text-destructive">
                                                            {form.formState.errors.lines[index]?.qtyInvoiced?.message}
                                                        </p>
                                                    ) : null}
                                                </td>
                                                <td className="px-3 py-3">
...
function buildDefaultValues(
    purchaseOrder: Pick<PurchaseOrderDetail, 'currency' | 'supplierName' | 'lines' | 'poNumber' | 'id'>,
...
function convertLine(line: PurchaseOrderLine) {
    ...
    const unitPriceMajor =
        typeof line.unitPriceMinor === 'number' ? line.unitPriceMinor / 100 : typeof line.unitPrice === 'number' ? line.unitPrice : 0;
