...
const SCORE_FILTER_OPTIONS: {
    ...
    { label: 'All quotes', value: 'all' },
    {
        label: 'Rank 1-3',
        value: 'top3',
        helper: 'Shows the top three suppliers by normalized rank.',
    },
    {
        label: 'Composite >= 80%',
        value: 'best',
        helper: 'Hide suppliers scoring below 80% overall.',
    },
];
...
export function QuoteCompareTable({
    ...
    const [statusFilter, setStatusFilter] = useState<Set<Quote['status']>>(
        () => new Set(),
    );
            ...
            shortlistMutation.mutate(
                {
                    ...
                    rfqId: rfqContext ?? quote.rfqId ?? rfqId,
                },
                {
                    onSuccess: (updated) => {
                        publishToast({
            ...
            candidateRows.filter((row) => {
                ...
                if (supplierQuery.trim()) {
                    ...
                    if (
                        !supplierName
                            .toLowerCase()
                            .includes(supplierQuery.trim().toLowerCase())
                    ) {
                        return false;
                    }
                }
                ...
                if (statusFilter.size > 0) {
                    if (
                        !row.quote.status ||
                        !statusFilter.has(row.quote.status)
                    ) {
                        return false;
                    }
                }
    ...
    const quotesForMatrix = useMemo(
        () => sortedRows.map((row) => row.quote),
        [sortedRows],
    );
    ...
    const quoteItemIndex = useMemo(
        () => buildQuoteItemIndex(sortedRows),
        [sortedRows],
    );
    const lineOutliers = useMemo(
        () => computeLineOutlierStats(sortedRows),
        [sortedRows],
    );
    ...
    const filtersActive =
        ...
        scoreFilter !== 'all';
    const selectedScoreHelper = SCORE_FILTER_OPTIONS.find(
        (option) => option.value === scoreFilter,
    )?.helper;
    ...
    const showFallbackNotice =
        comparisonRows.length === 0 && comparisonQuery.isSuccess;
        ...
        <Sheet open={open} onOpenChange={onOpenChange}>
            ...
            >
                <SheetHeader className="px-4 pt-4">
                    ...
                    <SheetDescription>
                        Review supplier responses, sort by normalized scores,
                        and inspect line-level coverage before awarding.
                    </SheetDescription>
                </SheetHeader>
                ...
                <div className="flex flex-col gap-4 overflow-hidden px-4 pb-6">
                    {!hasEnoughQuotes ? (
                        <div className="rounded-xl border border-dashed border-muted-foreground/40 bg-muted/20 px-4 py-6 text-center text-sm text-muted-foreground">
                            Select at least two quotes with the compare toggle
                            to open this view.
                        </div>
                    ) : (
                        <Fragment>
                            <div className="flex flex-wrap items-center justify-between gap-3">
                                ...
                                <div className="flex flex-wrap items-center gap-3">
                                    ...
                                    <Button
                                        ...
                                        title={
                                            canLaunchAwardFlow
                                                ? 'Review winners and convert to POs'
                                                : 'Select at least one quote to continue'
                                        }
                                    >
                            ...
                            {comparisonQuery.isError ? (
                                <div className="rounded-lg border border-destructive/30 bg-destructive/10 px-4 py-3 text-sm text-destructive">
                                    Unable to load normalized scores. Showing
                                    raw quote data.
                                </div>
                            ) : null}
                            ...
                            {showFallbackNotice ? (
                                <div className="rounded-lg border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-900">
                                    Scores will appear once the comparison
                                    service responds. Raw quote totals are shown
                                    for now.
                                </div>
                            ) : null}
                            ...
                            {!hasFilterMatches ? (
                                <div className="rounded-xl border border-dashed border-muted-foreground/40 bg-muted/20 px-4 py-6 text-center text-sm text-muted-foreground">
                                    ...
                                    {filtersActive
                                        ? 'Reset filters to see all suppliers.'
                                        : ''}
                                </div>
                            ) : (
                                <Fragment>
                                    <div className="grid gap-3 md:grid-cols-[minmax(280px,1fr)_repeat(auto-fit,minmax(260px,1fr))]">
                                        {sortedRows.map((row) => {
                                            ...
                                            const entityIdForBadge =
                                                Number.isNaN(parsedEntityId)
                                                    ? undefined
                                                    : parsedEntityId;
                                            ...
                                            return (
                                                <div
                                                    ...
                                                    className={cn(
                                                        ...
                                                        isShortlisted &&
                                                            'border-emerald-500/70 bg-emerald-50/80',
                                                    )}
                                                >
                                                    ...
                                                    <div className="grid gap-2">
                                                        ...
                                                        {canViewSupplierRisk ? (
                                                            <LazySupplierRiskBadge
                                                                ...
                                                                supplier={buildQuoteSupplierRiskPayload(
                                                                    quote,
                                                                )}
                                                                entityType="quote"
                                                                entityId={
                                                                ...
                                                                {quotesForMatrix.map(
                                                                    (quote) => {
                                                                        ...
                                                                        const moqLabel =
                                                                            minOrderQty !==
                                                                            null
                                                                                ? `MOQ ${formatNumber(minOrderQty)}`
                                                                                : null;
                                                                        const moqExplanation =
                                                                            minOrderQty !==
                                                                                null &&
                                                                            requestedQty !==
                                                                                null &&
                                                                            minOrderQty >
                                                                                requestedQty
                                                                                ? `Minimum order quantity exceeds requested qty ${formatNumber(
                                                                                      requestedQty,
                                                                                  )}${line.uom ? ` ${line.uom}` : ''}.`
                                                                                : null;
...
function buildQuoteItemIndex(
    rows: QuoteComparisonRow[],
): Map<string, Map<string, QuoteItem>> {
...
function computeLineOutlierStats(
    rows: QuoteComparisonRow[],
): Map<string, LineOutlierStats> {
...
function OutlierBadge({
    ...
    if (kind === 'price') {
        if (stats.priceLow != null && value < stats.priceLow) {
            const low = formatCurrencyMinor(
                stats.priceLow,
                currency ?? undefined,
            );
            const high = formatCurrencyMinor(
                stats.priceHigh ?? stats.priceLow,
                currency ?? undefined,
            );
            return (
                <Tooltip>
                    ...
                    <TooltipContent>
                        <p>
                            Unit price is below the typical band of {low} -{' '}
                            {high}.
                        </p>
                    </TooltipContent>
                </Tooltip>
        ...
        if (stats.priceHigh != null && value > stats.priceHigh) {
            ...
            return (
                <Tooltip>
                    ...
                    <TooltipContent>
                        <p>
                            Unit price is above the typical band of {low} -{' '}
                            {high}.
                        </p>
                    </TooltipContent>
                </Tooltip>
        ...
        if (stats.leadLow != null && value < stats.leadLow) {
            return (
                <Tooltip>
                    ...
                    <TooltipContent>
                        <p>
                            Lead time is faster than the typical band of{' '}
                            {Math.round(stats.leadLow)} -{' '}
                            {Math.round(stats.leadHigh ?? stats.leadLow)} days.
                        </p>
                    </TooltipContent>
                </Tooltip>
            );
        ...
        if (stats.leadHigh != null && value > stats.leadHigh) {
            return (
                <Tooltip>
                    ...
                    <TooltipContent>
                        <p>
                            Lead time is slower than the typical band of{' '}
                            {Math.round(stats.leadLow ?? stats.leadHigh)} -{' '}
                            {Math.round(stats.leadHigh)} days.
                        </p>
                    </TooltipContent>
                </Tooltip>
            );
...
function BandHint({
    ...
    if (kind === 'price') {
        if (stats.priceLow == null || stats.priceHigh == null) {
            return null;
        }

        const low = formatCurrencyMinor(stats.priceLow, currency ?? undefined);
        const high = formatCurrencyMinor(
            stats.priceHigh,
            currency ?? undefined,
        );
        ...
        return (
            <Tooltip>
                ...
                <TooltipContent>
                    <p>
                        Band uses median +/- 20% across submitted quotes for
                        this line.
                    </p>
                </TooltipContent>
            </Tooltip>
    ...
    if (stats.leadLow == null || stats.leadHigh == null) {
        return null;
    }

    const low = Math.round(stats.leadLow);
    const high = Math.round(stats.leadHigh);
    ...
    return (
        <Tooltip>
            ...
            <TooltipContent>
                <p>
                    Band uses median +/- 20% across submitted quotes for this
                    line.
                </p>
            </TooltipContent>
        </Tooltip>
...
function buildQuoteSupplierRiskPayload(quote: Quote) {
    ...
    return {
        ...
        supplier_company_id:
            (supplierDetails as { companyId?: number | null })?.companyId ??
            null,
        supplier_company_name:
            (supplierDetails as { companyName?: string | null })?.companyName ??
            null,
        currency: quote.currency,
