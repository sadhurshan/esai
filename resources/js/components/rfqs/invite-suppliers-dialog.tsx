...
const inviteSchema = z.object({
    supplierIds: z.string().min(1, 'Provide at least one supplier email or identifier.'),
...
export function InviteSuppliersDialog({ open, onOpenChange, rfqId }: InviteSuppliersDialogProps) {
    ...
    const canInviteSuppliers = featureFlagsLoaded ? hasFeature('rfqs.suppliers.invite') : true;
    const canBrowseSupplierDirectory = featureFlagsLoaded ? hasFeature('suppliers.directory.browse') : true;
    ...
    const supplierIdsRaw = useWatch({ control: form.control, name: 'supplierIds' });
    ...
    const handleSupplierSelectedFromDirectory = (supplier: Supplier) => {
        ...
        const existing = splitSupplierEntries(form.getValues('supplierIds') ?? '');
    ...
    const handleSubmit = form.handleSubmit(async (values) => {
        ...
        if (!canInviteSuppliers) {
            publishToast({
                ...
                description: 'Upgrade your plan to invite suppliers from the directory.',
            });
            return;
        }

        const uniqueSupplierIds = Array.from(new Set(splitSupplierEntries(values.supplierIds)));
        ...
        <Dialog open={open} onOpenChange={handleDialogOpenChange}>
            <DialogContent>
                <DialogHeader>
                    ...
                    <DialogDescription>
                        Paste supplier emails or directory identifiers, one per line. Duplicate entries will be removed
                ...
                <form className="grid gap-4" onSubmit={handleSubmit}>
                    <div className="grid gap-2">
                        <div className="flex items-center justify-between gap-2">
                            <Label htmlFor="supplierIds">Supplier emails or IDs</Label>
                        ...
                        <Textarea
                            id="supplierIds"
                            rows={5}
                            placeholder={['supplier@example.com', 'SUP-1234'].join('\n')}
                    ...
                    <div className="text-xs text-muted-foreground">
                        {parsedSuppliers.length} recipient{parsedSuppliers.length === 1 ? '' : 's'} will be invited.
                    </div>

                    <DialogFooter>
                        <Button type="submit" disabled={isSubmitting || !canInviteSuppliers}>
                            Send invitations
                        </Button>
                    </DialogFooter>
                </form>
