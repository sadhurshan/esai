...
function toPersonaDisplay<
    T extends {
        type: string;
        company_name?: string | null;
        supplier_company_name?: string | null;
    },
>(persona: T, forceSupplier: boolean): T & { type: PersonaType } {
    const normalizedType: PersonaType = forceSupplier
        ? 'supplier'
...
function formatPersonaTitle(persona: {
    type: 'buyer' | 'supplier';
    company_name?: string | null;
    supplier_company_name?: string | null;
}): string {
...
export function TopBar() {
    ...
    const supplierBadgeLabel = useMemo(() => {
        ...
        const hostBuyerName =
            activePersona.company_name ?? activePersona.supplier_company_name;
        if (!hostBuyerName) {
    ...
    return (
        <header className="flex h-16 items-center gap-4 border-b bg-background px-4">
            ...
            <div className="flex items-center gap-3">
                {personas.length > 0 ? (
                    <DropdownMenu>
                        <DropdownMenuTrigger asChild>
                            <Button
                                variant="outline"
                                className="relative hidden w-64 flex-col items-start gap-0 pr-8 text-left md:flex"
                            >
                ...
                {isPlatformOperator || isSupplierPersona ? null : (
                    <NotificationBell />
                )}
