...
export function useDeleteWebhook(): UseMutationResult<void, unknown, DeleteWebhookInput> {
    const adminConsoleApi = useSdkClient(AdminConsoleApi);
    const queryClient = useQueryClient();

    return useMutation<void, unknown, DeleteWebhookInput>({
        mutationFn: async ({ subscriptionId }) => adminConsoleApi.deleteWebhook(subscriptionId),
        onSuccess: (_data, variables) => {
            successToast('Webhook deleted', 'Subscription removed successfully.');
