...
export function useDeleteAward(): UseMutationResult<RfqItemAwardSummary[], unknown, DeleteAwardInput> {
    ...
    return useMutation<RfqItemAwardSummary[], unknown, DeleteAwardInput>({
        mutationFn: async ({ awardId }) => {
            if (!Number.isFinite(awardId) || awardId <= 0) {
                throw new Error('A valid award id is required to delete an award.');
            }
