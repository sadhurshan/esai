...
export function useCreateCreditNote(): UseMutationResult<CreditNoteDetail, HttpError | Error, CreateCreditNoteInput> {
    const creditApi = useSdkClient(CreditApi);
    const queryClient = useQueryClient();

    return useMutation<CreditNoteDetail, HttpError | Error, CreateCreditNoteInput>({
        mutationFn: async (input) => {
            if (!Number.isFinite(input.invoiceId) || input.invoiceId <= 0) {
                throw new Error('A valid invoice reference is required to create a credit note.');
            }
            ...
            const response = (await creditApi.createCreditNoteFromInvoice(payload)) as Record<string, unknown>;
        ...
        onSuccess: (creditNote) => {
            ...
            onError: (error) => {
                ...
                publishToast({
                    ...
                    description: error.message ?? 'Unable to create the credit note.',
                });
            },
    });
}
