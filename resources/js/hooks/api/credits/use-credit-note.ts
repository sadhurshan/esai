...
export function useCreditNote(
    creditNoteId?: number | string | null,
): UseQueryResult<CreditNoteDetail, HttpError> {
    const creditApi = useSdkClient(CreditApi);
    const parsedId =
        creditNoteId !== null && creditNoteId !== undefined
            ? Number(creditNoteId)
            : NaN;
    ...
    return useQuery<Record<string, unknown>, HttpError, CreditNoteDetail>({
        ...
        enabled,
        queryFn: async () =>
            (await creditApi.getCreditNote(parsedId)) as Record<
                string,
                unknown
            >,
        select: (payload) =>
            mapCreditNoteDetail(isRecord(payload) ? payload : {}),
    });
}
