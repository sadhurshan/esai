...
export function useIssueCreditNote(): UseMutationResult<CreditNoteDetail, HttpError | Error, IssueCreditNoteInput> {
    const creditApi = useSdkClient(CreditApi);
    const queryClient = useQueryClient();

    return useMutation<CreditNoteDetail, HttpError | Error, IssueCreditNoteInput>({
        mutationFn: async ({ creditNoteId }) => {
            ...
            const response = (await creditApi.issueCreditNote({ creditNoteId: parsedId })) as Record<string, unknown>;
