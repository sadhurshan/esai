...
export type UseAdminDigitalTwinCategoriesResult = UseQueryResult<AdminDigitalTwinCategoryNode[]> & {
...
export function useCreateAdminDigitalTwinCategory(
	...
	return useMutation({
		...
		onSuccess: (data, variables, onMutateResult, context) => {
			queryClient.invalidateQueries({ queryKey: queryKeys.digitalTwins.adminCategories() });
...
export function useUpdateAdminDigitalTwinCategory(
	...
	return useMutation({
		...
		mutationFn: async ({ categoryId, ...payload }) => api.updateCategory(categoryId, payload),
		...options,
		onSuccess: (data, variables, onMutateResult, context) => {
			queryClient.invalidateQueries({ queryKey: queryKeys.digitalTwins.adminCategories() });
			options.onSuccess?.(data, variables, onMutateResult, context);
