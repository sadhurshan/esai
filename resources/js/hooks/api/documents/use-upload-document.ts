    ...
    >({
        mutationFn: async ({
            ...
            if (file.size > MAX_BYTES) {
                throw new Error(
                    'Attachment exceeds the 50 MB limit. Compress the file and try again.',
                );
            }
            ...
            const response = await api.post<UploadResponse | DocumentPayload>(
                '/documents',
                formData,
                {
                    headers: {
            ...
            const payload = isUploadResponse(body)
                ? body.data
                : isDocumentPayload(body)
                  ? body
                  : undefined;
        ...
        onError: (error) => {
            ...
            publishToast({
                ...
                title: 'Upload failed',
                description:
                    error.message ?? 'Unable to upload the selected document.',
            });
        },
    });
}
