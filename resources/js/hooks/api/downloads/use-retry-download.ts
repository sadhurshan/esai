...
export function useRetryDownload(): UseMutationResult<DownloadJobSummary, ApiError, RetryDownloadVariables> {
    const queryClient = useQueryClient();

    return useMutation<DownloadJobSummary, ApiError, RetryDownloadVariables>({
        mutationFn: async ({ jobId }) => {
            const response = (await api.post<DownloadJobResponseItem>(`/downloads/${jobId}/retry`)) as unknown as DownloadJobResponseItem;
