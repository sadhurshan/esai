...
export function useInvoice(invoiceId?: string | number): UseQueryResult<InvoiceDetail, ApiError> {
    ...
    return useQuery<Record<string, unknown>, ApiError, InvoiceDetail>({
        ...
        enabled,
        queryFn: async () => {
            const response = (await api.get<Record<string, unknown>>(`/invoices/${id}`)) as unknown as Record<string, unknown>;
            return response;
        },
