...
export const use3WayMatch = (): UseMutationResult<Record<string, unknown>, HttpError, MatchResolutionInput> => {
    const queryClient = useQueryClient();
    const matchingApi = useSdkClient(MatchingApi);

    return useMutation<Record<string, unknown>, HttpError, MatchResolutionInput>({
        ...
        onSuccess: (_, variables) => {
            publishToast({
                ...
                description: 'Discrepancies have been reconciled across PO, GRN, and invoice.',
            });
        ...
        onError: (error) => {
            publishToast({
                ...
                description: error.message ?? 'Review discrepancies and try again.',
            });
        },
    });
};
