...
} from '@/hooks/api/usePurchaseOrder';
...
function extractPurchaseOrder(
    payload: AckPoEnvelope | PurchaseOrderResponse | undefined,
): PurchaseOrderResponse | undefined {
    if (!payload) {
        return undefined;
    }
...
export function useAckPo(): UseMutationResult<
    ...
    return useMutation<PurchaseOrderSummary | undefined, Error, AckPoInput>({
        mutationFn: async ({ poId, decision, reason }) => {
            if (!Number.isFinite(poId) || poId <= 0) {
                throw new Error(
                    'A valid purchase order id is required to respond.',
                );
            }
            ...
            const response = (await api.post<
                AckPoEnvelope | PurchaseOrderResponse
