...
> {
    ...
    return useMutation<PurchaseOrderSummary[], unknown, CreatePoInput>({
        mutationFn: async ({ awardIds }) => {
            if (!awardIds.length) {
                throw new Error(
                    'Select at least one awarded line before converting to PO.',
                );
            }

            const response =
                await purchaseOrdersApi.createPurchaseOrdersFromAwards({
        ...
        onSuccess: (_pos, { rfqId }) => {
            publishToast({
                ...
                description:
                    'Awarded lines converted to draft purchase order(s).',
            });
