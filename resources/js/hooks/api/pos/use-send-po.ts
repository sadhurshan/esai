...
export function useSendPo(): UseMutationResult<
    PurchaseOrderDelivery | undefined,
    Error,
    SendPoInput
> {
    ...
    return useMutation<PurchaseOrderDelivery | undefined, Error, SendPoInput>({
        mutationFn: async ({ poId, message, overrideEmail }) => {
            if (!Number.isFinite(poId) || poId <= 0) {
                throw new Error(
                    'A valid PO id is required to send to the supplier.',
                );
            }

            // TODO: switch to the TS SDK once the send endpoint accepts payloads in the generated client.
            const response = (await api.post<SendPoResponse>(
        ...
        onSuccess: (_, { poId }) => {
            publishToast({
                ...
                description:
                    'Supplier has been notified via email and webhook.',
            });
