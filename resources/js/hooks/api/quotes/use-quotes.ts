    ...
    return useQuery<ListQuotesForRfq200Response, HttpError, UseQuotesResult>({
        ...
        select: (response) => {
            ...
            const filteredItems = normalizedFilters.hasClientFilters
                ...
                : serverItems;
            const pagedItems = applyClientSidePagination(
                filteredItems,
                normalizedFilters,
            );
            ...
            return {
                ...
                isClientSideFiltered:
                    normalizedFilters.hasClientFilters ||
                    Boolean(
                        normalizedFilters.page || normalizedFilters.perPage,
                    ),
            };
        },
        staleTime: 30_000,
...
function normalizeFilters(filters: UseQuotesFilters): NormalizedQuoteFilters {
    const statuses = toArray(filters.statuses ?? filters.status).filter(
        Boolean,
    ) as QuoteStatusEnum[];
    const supplierId =
        filters.supplierId != null ? String(filters.supplierId) : undefined;
...
function applyClientFilters(
    ...
    return quotes.filter((quote) => {
        ...
        if (
            ...
            !filters.statuses.includes(quote.status)
        ) {
            return false;
        }
