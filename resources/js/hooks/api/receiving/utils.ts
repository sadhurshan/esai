...
export function mapGrnSummary(
    ...
    const poId =
        toNumber(
            pick(payload, 'po_id', 'purchase_order_id', 'purchaseOrderId'),
        ) ?? 0;
    const poNumber =
        toStringValue(
            pick(
    ...
    const supplierId =
        toNumber(pick(payload, 'supplier_id', 'supplierId')) ?? null;
    const supplierName =
        toStringValue(pick(payload, 'supplier_name', 'supplierName')) ?? null;
    const status = (pick(payload, 'status') as string | undefined) ?? 'draft';
    const receivedAt =
        toDateString(pick(payload, 'received_at', 'receivedAt')) ?? null;
    const postedAt =
        toDateString(pick(payload, 'posted_at', 'postedAt')) ?? null;
    const linesCount = toNumber(
        pick(payload, 'lines_count', 'linesCount', 'line_count'),
    );
    const attachmentsCount = toNumber(
        pick(payload, 'attachments_count', 'attachmentsCount'),
    );
...
export function mapGrnLine(payload: Record<string, unknown>): GrnLine {
    const poLineId =
        toNumber(pick(payload, 'po_line_id', 'poLineId', 'line_id')) ?? 0;
    const orderedQty =
        toNumber(
            pick(
                payload,
    ...
    const qtyReceived =
        toNumber(
            pick(payload, 'qty_received', 'qtyReceived', 'received_qty'),
        ) ?? 0;
    const previouslyReceived =
        toNumber(pick(payload, 'previously_received', 'received_to_date')) ??
        undefined;
    const remainingQty =
        toNumber(pick(payload, 'remaining_qty', 'remainingQty')) ?? undefined;
    const lineNo = toNumber(pick(payload, 'line_no', 'lineNo')) ?? undefined;
    const description =
        toStringValue(pick(payload, 'description', 'item_description')) ??
        undefined;
    const uom =
        toStringValue(pick(payload, 'uom', 'unit_of_measure')) ?? undefined;
    const unitPriceMinor =
        toNumber(pick(payload, 'unit_price_minor', 'unitPriceMinor')) ??
        undefined;
    const currency = toStringValue(pick(payload, 'currency')) ?? undefined;
    const variance = toStringValue(
        pick(payload, 'variance', 'variance_type'),
    ) as GrnLine['variance'];

    const ncrFlagSource = pick(payload, 'ncr_flag', 'ncrFlag');
    const ncrsSource = pick(payload, 'ncrs');
    const openNcrCount = toNumber(
        pick(payload, 'open_ncr_count', 'openNcrCount'),
    );
    ...
    return {
        ...
        ncrFlag:
            ncrFlagSource !== undefined ? Boolean(ncrFlagSource) : undefined,
        openNcrCount,
...
function mapNcr(payload: Record<string, unknown>): NcrRecord {
    ...
    return {
        ...
        grnId:
            toNumber(
                pick(payload, 'grn_id', 'grnId', 'goods_receipt_note_id'),
            ) ?? 0,
        poLineId:
            toNumber(
                pick(
        ...
        status:
            (toStringValue(pick(payload, 'status')) as NcrRecord['status']) ??
            'open',
        disposition: toStringValue(
            pick(payload, 'disposition'),
        ) as NcrRecord['disposition'],
...
const mapAttachment = (
    ...
    mime:
        toStringValue(pick(payload, 'mime', 'content_type', 'mime_type')) ??
        'application/octet-stream',
    sizeBytes: toNumber(pick(payload, 'size_bytes', 'sizeBytes', 'size')) ?? 0,
    createdAt:
        toDateString(pick(payload, 'created_at', 'createdAt')) ?? undefined,
    downloadUrl:
        toStringValue(pick(payload, 'download_url', 'downloadUrl')) ??
        undefined,
});

export function mapGrnDetail(
    payload: Record<string, unknown>,
): GoodsReceiptNoteDetail {
    const summary = mapGrnSummary(payload);
    const reference =
        toStringValue(pick(payload, 'reference', 'external_reference')) ??
        undefined;
    const notes =
        toStringValue(pick(payload, 'notes', 'note', 'memo')) ?? undefined;
    ...
    const timeline = Array.isArray(timelineSource)
        ? timelineSource.filter(isRecord).map((entry, index) => ({
              id: toStringValue(pick(entry, 'id')) ?? `timeline-${index}`,
              summary:
                  toStringValue(pick(entry, 'summary', 'label', 'title')) ??
                  'Event',
              occurredAt:
                  toDateString(
                      pick(
              ...
              actor: isRecord(entry.actor)
                  ? {
                        ...
                        name:
                            toStringValue(pick(entry.actor, 'name')) ??
                            undefined,
                    }
                  : undefined,
          }))
