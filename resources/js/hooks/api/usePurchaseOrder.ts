...
type PurchaseOrderPdfDocumentLike =
    | PurchaseOrderResponsePdfDocument
    | SdkPurchaseOrderPdfDocument;
type PurchaseOrderDeliveryLike = Partial<PurchaseOrderDelivery> &
    Record<string, unknown>;

function isSdkPurchaseOrder(
    payload: PurchaseOrderLike,
): payload is SdkPurchaseOrder {
    return 'companyId' in payload;
}

function isSdkPurchaseOrderLine(
    payload: PurchaseOrderLineLike,
): payload is SdkPurchaseOrderLine {
    return 'lineNo' in payload;
}

function isSdkChangeOrder(
    payload: PurchaseOrderChangeOrderLike,
): payload is SdkPoChangeOrder {
    return 'purchaseOrderId' in payload && 'changesJson' in payload;
}

function isSdkPdfDocument(
    payload: PurchaseOrderPdfDocumentLike,
): payload is SdkPurchaseOrderPdfDocument {
...
const readSentBy = (
    value: unknown,
): PurchaseOrderDelivery['sentBy'] | undefined => {
    if (!isRecord(value)) {
        return undefined;
    }
...
export const mapPurchaseOrderLine = (
    ...
    const unitPrice = isSdkPurchaseOrderLine(payload)
        ? (payload.unitPrice ??
          (payload.unitPriceMinor ?? 0) / DEFAULT_MINOR_FACTOR)
        : payload.unit_price;
    ...
    const lineSubtotalMinor = isSdkPurchaseOrderLine(payload)
        ? (payload.lineSubtotalMinor ??
          (unitPriceMinor ?? 0) * payload.quantity)
        : undefined;

    const lineTotalMinor = isSdkPurchaseOrderLine(payload)
        ? (payload.lineTotalMinor ??
          lineSubtotalMinor ??
          (unitPriceMinor ?? 0) * payload.quantity)
        : undefined;
    ...
    const invoicedQuantity =
        extra?.invoicedQuantity ?? extra?.invoiced_quantity;
    const remainingQuantity =
        extra?.remainingQuantity ?? extra?.remaining_quantity;
    const receivedQuantity =
        extra?.receivedQuantity ?? extra?.received_quantity;

    return {
        id: payload.id,
        lineNo: isSdkPurchaseOrderLine(payload)
            ? payload.lineNo
            : payload.line_no,
        description: isSdkPurchaseOrderLine(payload)
            ? (payload.description ?? '')
            : payload.description,
        quantity: payload.quantity,
        uom: isSdkPurchaseOrderLine(payload)
            ? (payload.uom ?? '')
            : payload.uom,
        unitPrice,
        unitPriceMinor,
        currency: isSdkPurchaseOrderLine(payload)
            ? payload.currency
            : undefined,
        lineSubtotalMinor,
        taxTotalMinor: isSdkPurchaseOrderLine(payload)
            ? payload.taxTotalMinor
            : undefined,
        lineTotalMinor,
        deliveryDate,
        invoicedQuantity:
            typeof invoicedQuantity === 'number' ? invoicedQuantity : undefined,
        remainingQuantity:
...
export const mapPurchaseOrder = (
    ...
    const supplier = isSdkPurchaseOrder(payload)
        ? payload.supplier
        : payload.supplier;
    const supplierEmail = (supplier as { email?: string | null } | undefined)
        ?.email;
    const rfq = isSdkPurchaseOrder(payload) ? payload.rfq : payload.rfq;
    const changeOrdersSource = (
        isSdkPurchaseOrder(payload)
            ? payload.changeOrders
            : payload.change_orders
    ) as PurchaseOrderChangeOrderLike[] | undefined;
    const linesSource = (payload.lines ?? []) as PurchaseOrderLineLike[];
    const mappedLines = linesSource.map(mapPurchaseOrderLine);
    const pdfDocumentId = isSdkPurchaseOrder(payload)
        ? (payload.pdfDocumentId ?? null)
        : (payload.pdf_document_id ?? null);
    const pdfDocumentSource = isSdkPurchaseOrder(payload)
        ? payload.pdfDocument
    ...
    const ackStatus = (ackStatusRaw ??
        'draft') as PurchaseOrderSummary['ackStatus'];
    const sentAt = toMaybeIsoString(pickDateLike(extra, 'sentAt', 'sent_at'));
    const acknowledgedAt = toMaybeIsoString(
        pickDateLike(extra, 'acknowledgedAt', 'acknowledged_at'),
    );
    ...
    const deliveries = Array.isArray(deliveriesSource)
        ? (deliveriesSource as PurchaseOrderDeliveryLike[]).map(
              mapPurchaseOrderDelivery,
          )
        : undefined;
    const latestDeliverySource = pickValue(
        extra,
        'latestDelivery',
        'latest_delivery',
    );
    const latestDelivery = latestDeliverySource
        ? mapPurchaseOrderDelivery(
              latestDeliverySource as PurchaseOrderDeliveryLike,
          )
        : deliveries?.[0];

    const subtotalMinor = isSdkPurchaseOrder(payload)
        ? payload.subtotalMinor
        : undefined;
    const taxAmountMinor = isSdkPurchaseOrder(payload)
        ? payload.taxAmountMinor
        : undefined;
    let totalMinor = isSdkPurchaseOrder(payload)
        ? payload.totalMinor
        : undefined;

    if (totalMinor === undefined && mappedLines.length > 0) {
        totalMinor = mappedLines.reduce(
            (acc, line) =>
                acc +
                (line.lineTotalMinor ??
    ...
    const derivedTax = mappedLines.reduce(
        (acc, line) => acc + (line.taxTotalMinor ?? 0),
        0,
    );

    return {
        id: payload.id,
        companyId: isSdkPurchaseOrder(payload)
            ? payload.companyId
            : payload.company_id,
        poNumber: isSdkPurchaseOrder(payload)
            ? payload.poNumber
            : payload.po_number,
        ...
        taxPercent:
            (isSdkPurchaseOrder(payload)
                ? payload.taxPercent
                : payload.tax_percent) ?? undefined,
        revisionNo: isSdkPurchaseOrder(payload)
            ? (payload.revisionNo ?? 0)
            : payload.revision_no,
        rfqId: isSdkPurchaseOrder(payload)
            ? (payload.rfqId ?? null)
            : payload.rfq_id,
        quoteId: isSdkPurchaseOrder(payload)
        ...
        ackReason,
        subtotalMinor:
            subtotalMinor ?? (mappedLines.length ? derivedSubtotal : undefined),
        taxAmountMinor:
            taxAmountMinor ?? (mappedLines.length ? derivedTax : undefined),
        totalMinor,
        lines: mappedLines,
        ...
        pdfDocument:
            mappedPdfDocument ??
            (pdfDocumentSource === null ? null : undefined),
        deliveries,
        latestDelivery,
    };
};
...
export const mapChangeOrder = (
    ...
    id: payload.id,
    purchaseOrderId: isSdkChangeOrder(payload)
        ? payload.purchaseOrderId
        : payload.purchase_order_id,
    reason: payload.reason,
    status: payload.status,
    changes: ((isSdkChangeOrder(payload)
        ? payload.changesJson
        : payload.changes_json) ?? {}) as Record<string, unknown>,
    poRevisionNo:
        (isSdkChangeOrder(payload)
            ? payload.poRevisionNo
            : payload.po_revision_no) ?? undefined,
    proposedBy: (() => {
...
const mapPurchaseOrderDelivery = (
    payload: PurchaseOrderDeliveryLike,
): PurchaseOrderDelivery => {
    const source = payload as Record<string, unknown>;
    const channel = (payload.channel ??
        (pickString(source, 'channel') as PurchaseOrderDelivery['channel']) ??
        'email') as PurchaseOrderDelivery['channel'];
    const status = (payload.status ??
        (pickString(source, 'status') as PurchaseOrderDelivery['status']) ??
        'pending') as PurchaseOrderDelivery['status'];
    const responseMeta =
        payload.responseMeta ?? pickRecord(source, 'response_meta');
    const sentBy =
        payload.sentBy ??
        readSentBy(pickValue(source, 'sent_by')) ??
        readSentBy(pickValue(source, 'creator')) ??
        null;
    ...
    return {
        ...
        status,
        recipientsTo:
            payload.recipientsTo ??
            toStringArrayOrNull(pickValue(source, 'recipients_to')),
        recipientsCc:
            payload.recipientsCc ??
            toStringArrayOrNull(pickValue(source, 'recipients_cc')),
        message: payload.message ?? pickString(source, 'message'),
        deliveryReference:
            payload.deliveryReference ??
            pickString(source, 'deliveryReference', 'delivery_reference'),
        responseMeta,
        errorReason:
            payload.errorReason ??
            pickString(source, 'errorReason', 'error_reason'),
        sentAt:
            payload.sentAt ??
            toMaybeIsoString(pickDateLike(source, 'sentAt', 'sent_at')) ??
            null,
        createdAt:
            payload.createdAt ??
            toMaybeIsoString(pickDateLike(source, 'createdAt', 'created_at')) ??
            null,
        updatedAt:
            payload.updatedAt ??
            toMaybeIsoString(pickDateLike(source, 'updatedAt', 'updated_at')) ??
            null,
        sentBy,
    };
};
