...
export function useSupplierDocuments(): UseQueryResult<
    SupplierDocumentCollection,
    ApiError
> {
    return useQuery<SupplierDocumentCollection, ApiError>({
        queryKey: queryKeys.me.supplierDocuments(),
        queryFn: async () =>
            (await api.get<SupplierDocumentCollection>(
                '/me/supplier-documents',
            )) as unknown as SupplierDocumentCollection,
        staleTime: 15_000,
    });
}

export function useUploadSupplierDocument(): UseMutationResult<
    SupplierDocument,
    ApiError,
    ...
    return useMutation<SupplierDocument, ApiError, UploadSupplierDocumentInput>(
        {
            mutationFn: async (input) => {
                ...
                return (await api.post<SupplierDocument>(
                    '/me/supplier-documents',
                    formData,
                    {
...
export function useDeleteSupplierDocument(): UseMutationResult<
    void,
    ApiError,
    number
> {
