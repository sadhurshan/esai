...
export function createRealtimeClient({
    token,
}: CreateRealtimeClientOptions): EchoInstance | null {
    ...
    if (!appKey) {
        if (import.meta.env.DEV && !warnedMissingKey) {
            console.warn(
                'VITE_PUSHER_APP_KEY is not set. Realtime notifications are disabled.',
            );
    ...
    const port = Number(
        import.meta.env.VITE_PUSHER_PORT ?? (customHost ? 6001 : defaultPort),
    );
