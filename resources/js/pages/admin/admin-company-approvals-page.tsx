...
export function AdminCompanyApprovalsPage() {
    ...
    const [selectedCompany, setSelectedCompany] =
        useState<CompanyApprovalItem | null>(null);
    ...
    const selectedCompanyIsUk = selectedCompany
        ? isUkCountry(selectedCompany.country)
        : false;
    const companiesHouseEligible =
        selectedCompanyIsUk && Boolean(selectedCompany?.registration_no);
    ...
    const documentsErrorMessage = companyDocumentsQuery.isError
        ? (companyDocumentsQuery.error?.message ??
          'Unable to load registration documents.')
        : null;
    ...
    const activeFilter =
        ...
        STATUS_FILTERS[0];
    const rejectingId =
        rejectMutation.isPending && rejectModal.company
            ? rejectModal.company.id
            : null;
    const detailEligible = selectedCompany
        ? isCompanyModerationEligible(selectedCompany.status)
        : false;
    const detailAwaitingApproval = selectedCompany
        ? approvingId === selectedCompany.id
        : false;
    const detailRejecting = selectedCompany
        ? rejectMutation.isPending &&
          rejectModal.company?.id === selectedCompany.id
        : false;
    ...
    return (
        <div className="space-y-8">
            ...
            >
                <DialogContent className="flex max-h-[90vh] max-w-3xl flex-col">
                    <DialogHeader>
                        ...
                        <DialogDescription>
                            Verify the submitted registration details before
                            approving or rejecting the tenant.
                        </DialogDescription>
                    </DialogHeader>
                    {selectedCompany ? (
                            ...
                            <div className="space-y-6">
                                ...
                                <DetailSection title="Primary contact">
                                    <div className="grid gap-3 sm:grid-cols-2">
                                        <DetailField
                                            label="Name"
                                            value={
                                                selectedCompany.primary_contact_name ??
                                                '—'
                                            }
                                        />
                                        <DetailField
                                        ...
                                        <DetailField
                                            label="Phone"
                                            value={
                                                selectedCompany.primary_contact_phone ??
                                                '—'
                                            }
                                        />
                                    </div>
                                ...
                                <DetailSection title="Compliance & verification">
                                    <div className="grid gap-3 sm:grid-cols-2">
                                        <DetailField
                                            label="Registration #"
                                            value={
                                                selectedCompany.registration_no ??
                                                '—'
                                            }
                                        />
                                        <DetailField
                                        ...
                                        <DetailField
                                            label="Verified by"
                                            value={
                                                selectedCompany.verified_by
                                                    ? `User #${selectedCompany.verified_by}`
                                                    : '—'
                                            }
                                        />
                                        <DetailField
                                            ...
                                            value={formatDate(
                                                selectedCompany.supplier_profile_completed_at,
                                            )}
                                        />
                                    </div>
                                ...
                                <DetailSection title="Companies House verification">
                                    {!companiesHouseEligible ? (
                                        <p className="text-sm text-muted-foreground">
                                            ...
                                            companies that provided a
                                            registration number.
                                        </p>
                                    ) : companiesHouseQuery.isLoading ? (
                                        <p className="text-sm text-muted-foreground">
                                            Fetching Companies House profile…
                                        </p>
                                        ...
                                        <CompaniesHouseProfileSummary
                                            profile={companiesHouseProfile}
                                        />
                                    ) : (
                                        <p className="text-sm text-muted-foreground">
                                            No Companies House record was found
                                            for registration #
                                            {selectedCompany.registration_no}
                                        </p>
                                    )}
                                ...
                                <DetailSection title="Registration documents">
                                    {documentsLoading ? (
                                        ...
                                        <p className="text-sm text-muted-foreground">
                                            No documents were uploaded with this
                                            registration.
                                        </p>
                                    ) : (
                        ...
                        <DialogFooter className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                            ...
                            <div className="flex flex-col gap-2 sm:flex-row">
                                ...
                                <Button
                                    ...
                                    onClick={() =>
                                        handleRejectFromDetails(selectedCompany)
                                    }
                                >
            ...
            <Dialog
                open={Boolean(rejectModal.company)}
                onOpenChange={(open) => {
                    if (!open) {
                        closeRejectDialog();
                    }
            ...
            >
                <DialogContent className="max-w-lg">
                    <DialogHeader>
                        <DialogTitle>
                            Reject {rejectModal.company?.name ?? 'company'}
                        </DialogTitle>
                        <DialogDescription>
                            Provide a short reason so the tenant understands how
                            to remediate their submission.
                        </DialogDescription>
                    </DialogHeader>
                    <div className="space-y-3">
                        <div className="space-y-2">
                            ...
                            />
                            <p className="text-xs text-muted-foreground">
                                Minimum 5 characters. This note is shared with
                                the tenant owner.
                            </p>
                        </div>
                    </div>
                    <DialogFooter className="flex flex-col gap-2 sm:flex-row sm:justify-end">
                        ...
                        <Button
                            ...
                            variant="destructive"
                            disabled={
                                !reasonIsValid || rejectMutation.isPending
                            }
...
function CompaniesHouseProfileSummary({
    profile,
}: {
    profile: CompaniesHouseProfile;
}) {
    const address = formatCompaniesHouseAddress(
    ...
    return (
        <div className="space-y-4">
            ...
            <div className="grid gap-3 sm:grid-cols-2">
                ...
                <DetailField
                    label="SIC codes"
                    value={formatSicCodes(profile.sic_codes)}
                />
                <DetailField
                    label="Can file"
                    value={formatCompaniesHouseBoolean(profile.can_file)}
                />
                <DetailField
                    label="Has been liquidated"
                    value={formatCompaniesHouseBoolean(
            ...
            {profile.retrieved_at ? (
                <p className="text-xs text-muted-foreground">
                    Synced {formatDate(profile.retrieved_at)}
                </p>
            ) : null}
        </div>
    );
...
function isUkCountry(country?: string | null): boolean {
    ...
    return UK_COUNTRY_IDENTIFIERS.some(
        (identifier) =>
            normalized === identifier || normalized.includes(identifier),
    );
}

function formatCompaniesHouseAddress(
    address?: CompaniesHouseProfile['registered_office_address'],
): string | null {
    if (!address) {
        return null;
    }
...
function formatPreviousNameRange(
    entry: NonNullable<CompaniesHouseProfile['previous_company_names']>[number],
): string {
    const parts: string[] = [];
...
function formatDocumentType(type?: string | null): string {
    ...
    return (
        ...
        type.replace(/_/g, ' ').replace(/^./, (char) => char.toUpperCase())
    );
}
