...
import type { AdminDigitalTwinCategoryNode, AdminDigitalTwinListItem } from '@/sdk';
import { HttpError } from '@/sdk';

const STATUS_OPTIONS: Array<{ value: AdminDigitalTwinStatusFilter; label: string }> = [
...
export function AdminDigitalTwinListPage() {
	...
	const [statusFilter, setStatusFilter] = useState<AdminDigitalTwinStatusFilter>('all');
	const [categoryFilter, setCategoryFilter] = useState<string>('all');
	const [cursor, setCursor] = useState<string | undefined>(undefined);
	const [pendingDelete, setPendingDelete] = useState<AdminDigitalTwinListItem | null>(null);
	...
	const { items, meta, isLoading, isFetching, error, refetch } = useAdminDigitalTwins({
		cursor,
			...
			<Heading
				...
				action={
					<div className="flex flex-wrap gap-2">
						...
						<Button variant="outline" onClick={() => navigate('/app/admin/digital-twins/categories')}>
							<FolderTree className="mr-2 h-4 w-4" /> Manage categories
						</Button>
						<Button onClick={() => navigate('/app/admin/digital-twins/new')}>
							<PlusCircle className="mr-2 h-4 w-4" /> New digital twin
						</Button>
					</div>
				}
			/>
			...
			<Card>
				<CardContent className="space-y-4 pt-6">
					<div className="grid gap-4 md:grid-cols-3">
						...
						<div className="space-y-2">
							<Label htmlFor="digital-twin-status-filter">Status</Label>
							<Select value={statusFilter} onValueChange={(value: AdminDigitalTwinStatusFilter) => setStatusFilter(value)}>
						...
						<div className="space-y-2">
							<Label htmlFor="digital-twin-category-filter">Category</Label>
			...
			<Card>
				<CardContent className="p-0">
											...
											<td className="px-4 py-4 align-top text-right">
												<DropdownMenu>
													<DropdownMenuTrigger asChild>
														<Button variant="ghost" size="icon" aria-label="Digital twin actions">
													...
													<DropdownMenuContent align="end" className="w-48">
														...
														<DropdownMenuItem
															onClick={() =>
																item.status === 'published' ? handleArchive(item) : handlePublish(item)
															}
														>
					...
					<div className="flex items-center justify-between border-t px-4 py-3 text-sm text-muted-foreground">
						<span>
							{items.length} result{items.length === 1 ? '' : 's'} · Page size {meta?.perPage ?? '—'}
						</span>
...
function flattenCategories(nodes: AdminDigitalTwinCategoryNode[], depth = 0): Array<{ value: number; label: string }> {
