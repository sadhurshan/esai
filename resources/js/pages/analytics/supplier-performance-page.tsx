...
export function SupplierPerformancePage() {
    const { hasFeature, notifyPlanLimit, clearPlanLimit, activePersona } =
        useAuth();
    ...
    const personaSupplierName =
        activePersona?.supplier_name ?? activePersona?.company_name ?? null;

    const [filterDraft, setFilterDraft] = useState<FilterFormState>(() =>
        createDefaultFilters(personaSupplierId),
    );
    ...
    const supplierSearchTerm = useDebouncedValue(
    ...
    useEffect(() => {
        if (!analyticsEnabled) {
            notifyPlanLimit({
                ...
                message:
                    'Upgrade your plan to unlock supplier performance analytics.',
            });
    ...
    const planForbidden =
        ...
        isForbiddenError(performanceQuery.error, 'analytics_upgrade_required');
    const permissionDenied =
        performanceQuery.isError &&
        isForbiddenError(
            performanceQuery.error,
            'supplier_performance_permission_required',
        );
    ...
    const keyMetrics = useMemo(
        () => buildKeyMetrics(report, formatNumber),
        [report, formatNumber],
    );

    const isLoading =
        performanceQuery.isLoading || performanceQuery.isPlaceholderData;
    const hasData = Boolean(
        report && (report.series.length > 0 || report.table.length > 0),
    );
    const errorMessage = performanceQuery.isError
        ? buildErrorMessage(performanceQuery.error)
        : null;
    ...
    const handleReset = useCallback(() => {
        const defaults = createDefaultFilters(
            isSupplierPersona ? personaSupplierId : null,
        );
            ...
            <div className="flex flex-1 flex-col gap-6">
                ...
                <div className="space-y-1">
                    ...
                    <p className="max-w-2xl text-sm text-muted-foreground">
                        Analyze on-time delivery, defect rates, and risk only
                        after enabling analytics in your plan.
                    </p>
                </div>
                <PlanUpgradeBanner />
                <EmptyState
        ...
        return (
            <div className="flex flex-1 flex-col gap-6">
                ...
                <div className="space-y-1">
                    ...
                    <p className="max-w-2xl text-sm text-muted-foreground">
                        Supplier analytics are disabled on the current plan.
                        Upgrade to unlock KPI scorecards and AI narratives.
                    </p>
                </div>
                <PlanUpgradeBanner />
                <EmptyState
        ...
        return (
            <div className="flex flex-1 flex-col gap-6">
                ...
                <div className="space-y-1">
                    ...
                    <p className="max-w-2xl text-sm text-muted-foreground">
                        Your role is missing the View supplier performance
                        permission required to open this workspace.
                    </p>
                </div>
                <EmptyState
        ...
        <div className="flex flex-1 flex-col gap-6">
            ...
            <div className="flex flex-wrap items-center justify-between gap-4">
                <div className="space-y-1">
                    ...
                    <p className="max-w-2xl text-sm text-muted-foreground">
                        Track quality, delivery, responsiveness, and price
                        signals with AI summaries tailored to each supplier.
                    </p>
                </div>
            ...
            <Card>
                <CardHeader>
                    ...
                    <CardDescription>
                        Narrow the analysis by time range and, if needed,
                        supplier.
                    </CardDescription>
                </CardHeader>
                ...
                <AlertDescription>
                    {filterChips.length ? (
                        ...
                        <span className="text-sm text-muted-foreground">
                            Using the default 90-day window.
                        </span>
                    )}
                </AlertDescription>
            </Alert>
                    ...
                    <div className="grid gap-4 xl:grid-cols-[1.6fr_1fr]">
                        <Card className="h-full">
                            <CardHeader className="space-y-1">
                                ...
                                <CardDescription>
                                    Generated for the selected supplier and date
                                    range.
                                </CardDescription>
                            </CardHeader>
                            <CardContent>
                                {isLoading ? (
                                  ...
                                  (summary.summaryMarkdown ||
                                    ...
                                    <p className="text-sm text-muted-foreground">
                                        No AI summary yet. Apply filters and
                                        refresh.
                                    </p>
                                )}
                            </CardContent>
                        ...
                        <Card className="h-full">
                            <CardHeader className="space-y-1">
                                ...
                                <CardDescription>
                                    Aggregated values over the selected range.
                                </CardDescription>
                            </CardHeader>
                        ...
                        <CardHeader className="space-y-1">
                            <div className="flex items-center justify-between gap-2">
                                <div>
                                    ...
                                    <CardDescription>
                                        Toggle metrics to focus on the most
                                        relevant signals.
                                    </CardDescription>
                                </div>
                            </div>
                        ...
                        <CardContent>
                            <PerformanceMultiChart
                                ...
                                valueFormatter={(value, key) =>
                                    formatMetricValue(value, key, formatNumber)
                                }
                            />
                        </CardContent>
                    </Card>
                        ...
                        <CardHeader>
                            <div className="flex items-center justify-between gap-3">
                                <div>
                                    ...
                                    <CardDescription>
                                        Accuracy, responsiveness, and risk
                                        snapshot.
                                    </CardDescription>
                                </div>
                                <Badge
                                ...
                                >
                                    ...
                                    {report?.filtersUsed.bucket ?? 'weekly'}{' '}
                                    cadence
                                </Badge>
                            </div>
                        </CardHeader>
...
function buildKeyMetrics(
    report: SupplierPerformanceReport | undefined,
    ...
    return [
        ...
        },
        {
            key: 'risk',
            label: 'Risk score',
            value: formatNumber(row.riskScore ?? 0, {
                maximumFractionDigits: 1,
            }),
            helper: row.riskCategory
...
function buildTableColumns(
    formatNumber: ReturnType<typeof useFormatting>['formatNumber'],
): Array<DataTableColumn<SupplierPerformanceTableRow>> {
    return [
        {
            key: 'supplierName',
            title: 'Supplier',
        ...
        },
        {
            ...
            align: 'right',
            render: (row) =>
                `${formatNumber(row.leadTimeVariance ?? 0, { maximumFractionDigits: 1 })}d`,
        },
        {
            key: 'priceVolatility',
        ...
        },
        {
            ...
            align: 'right',
            render: (row) =>
                formatNumber(row.riskScore ?? 0, { maximumFractionDigits: 1 }),
        },
        {
            key: 'riskCategory',
            title: 'Risk category',
