...
const registerSchema = z
    .object({
        startMode: z.enum(['buyer', 'supplier'], {
            required_error: 'Select how you want to start.',
        }),
        name: z
...
export function RegisterPage() {
    ...
    useEffect(() => {
        ...
        const looksUkLocale = languageCandidates.some((locale) =>
            locale.toLowerCase().includes('en-gb'),
        );
        const timeZone = Intl.DateTimeFormat().resolvedOptions().timeZone ?? '';
        const looksUkTimeZone =
            typeof timeZone === 'string' &&
            timeZone.toLowerCase().includes('europe/london');
    ...
    useEffect(() => {
        ...
        const runSearch = async () => {
            ...
            try {
                const response = (await api.get<CompaniesHouseSearchResponse>(
                    '/auth/companies-house/search',
                    {
                        params: {
                            q: query,
                            limit: 8,
                ...
                const message =
                    error instanceof Error
                        ? error.message
                        : 'Unable to search Companies House.';
    ...
    const handleCompanySelect = async (item: CompaniesHouseSearchItem) => {
        ...
        if (item.company_number) {
            setValue('registrationNo', item.company_number, {
                shouldDirty: true,
                shouldValidate: true,
            });
        }
        ...
        try {
            const response = (await api.get<CompaniesHouseProfileResponse>(
                '/auth/companies-house/profile',
                {
                    params: {
            ...
            const formattedAddress = formatCompaniesHouseAddress(
                profile?.registered_office_address ?? null,
            );
            ...
            const message =
                error instanceof Error
                    ? error.message
                    : 'Unable to load Companies House details.';
    ...
    const onSubmit = handleSubmit(async (values) => {
        ...
        if (!hasDocuments || missingFile) {
            setDocumentsError(
                'Upload at least one supporting document and attach a file for each row.',
            );
            return;
        }
        ...
        const applyServerErrors = (
            errorBag: Record<string, unknown> | undefined | null,
        ): string | null => {
            if (!errorBag) {
                return null;
            }
        ...
        try {
            ...
            const isSupplierStart =
                values.startMode === 'supplier' || result.needsSupplierApproval;
            ...
            if (error instanceof HttpError) {
                const body = error.body as
                    ...
                    | undefined;
                const validationMessage = applyServerErrors(
                    body?.errors ?? null,
                );
            ...
            <Card className="w-full max-w-4xl border border-white/10 bg-slate-950/80 text-slate-100 shadow-lg backdrop-blur">
                <CardHeader className="items-center space-y-2 text-center">
                    ...
                    <CardDescription className="text-slate-400">
                        The first user becomes the workspace owner. Buyer tools
                        are enabled immediately; supplier features activate
                        after approval.
                    </CardDescription>
                </CardHeader>
                <CardContent>
                    <form className="space-y-6" onSubmit={onSubmit} noValidate>
                        <div className="grid gap-4 md:grid-cols-2">
                            <div className="space-y-2">
                                ...
                                <Select
                                    ...
                                    onValueChange={(value) =>
                                        setValue(
                                            'startMode',
                                            value as RegisterFormValues['startMode'],
                                            { shouldDirty: true },
                                        )
                                    }
                                >
                                ...
                                <p className="text-xs text-slate-400">
                                    Buyer tools are enabled immediately;
                                    suppliers activate after approval.
                                </p>
                                {errors.startMode ? (
                                    <p className="text-xs text-destructive">
                                        {errors.startMode.message}
                                    </p>
                                ) : null}
                            </div>
                            <div className="space-y-2">
                            ...
                            <div className="space-y-2">
                                ...
                                {isUkRegistrant ? (
                                    ...
                                    <p className="text-xs text-slate-400">
                                        Set country to UK to enable Companies
                                        House search.
                                    </p>
                                ) : null}
                                {companyProfileLoading ? (
                                    <div className="flex items-center gap-2 text-xs text-slate-400">
                                        ...
                                        Fetching Companies House details…
                                    </div>
                                ) : null}
                                {companyProfileError ? (
                                    <p className="text-xs text-destructive">
                            ...
                            <div className="space-y-2">
                                ...
                                />
                                <p className="text-xs text-slate-400">
                                    Used for invitation policies and supplier
                                    vetting.
                                </p>
                        ...
                        <div className="space-y-2">
                            ...
                            <Textarea
                                className="rounded-sm border-slate-500 bg-transparent"
                                id="address"
                                rows={3}
                        ...
                        <div className="space-y-3">
                            <div>
                                ...
                                <p className="text-xs text-slate-400">
                                    Upload incorporation, tax, ESG, or other
                                ...
                                {documents.map((document) => (
                                    ...
                                    >
                                        <div className="space-y-2">
                                            ...
                                            >
                                                <SelectTrigger
                                                    className="rounded-sm border-slate-500 py-4"
                                        ...
                                        <div className="space-y-2">
                                            ...
                                            <div className="relative">
                                                <Input
                                                    ...
                                                    className="peer file:bg-brand-primary hover:file:bg-brand-primary/90 h-12 cursor-pointer border-slate-500 bg-slate-900/40 text-slate-200 file:mr-4 file:cursor-pointer file:rounded-sm file:border-0 file:px-4 file:py-2 file:text-sm file:font-medium file:text-white"
                                                    type="file"
                                                    accept={
                                                        DOCUMENT_INPUT_ACCEPT
                                                    }
                            ...
                            {documentsError ? (
                                ...
                                <p className="text-xs text-slate-400">
                                    Submit at least one document so the Elements
                                    Supply team can verify your company.
                                </p>
                            )}
                            <Button
                                type="button"
