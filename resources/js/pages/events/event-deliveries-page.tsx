...
export function EventDeliveriesPage() {
    ...
    const [detailItem, setDetailItem] = useState<EventDeliveryItem | null>(
        null,
    );
    ...
    const handleRetry = async (item: EventDeliveryItem) => {
        try {
            ...
            publishToast({
                title: 'Retry queued',
                description: `Delivery #${item.id} scheduled for retry.`,
                variant: 'success',
            });
        } catch (error) {
            const message =
    ...
    const handleReplay = async () => {
        ...
        try {
            ...
            publishToast({
                title: 'Replay queued',
                description:
                    'Selected dead-letter deliveries will be replayed shortly.',
                variant: 'success',
            });
            setSelectedDlq(new Set());
        } catch (error) {
            const message =
                error instanceof ApiError
                    ? error.message
                    : 'Unable to replay DLQ deliveries.';
            publishToast({
                title: 'Replay failed',
                description: message,
                variant: 'destructive',
            ...
            <div className="flex flex-wrap items-start justify-between gap-4">
                <div className="space-y-1">
                    ...
                    <p className="text-sm text-muted-foreground">
                        Inspect webhook attempts, retry failures, and replay
                        dead-lettered payloads across your company.
                    </p>
                </div>
            ...
            <Card className="border-border/70">
                <CardContent className="grid gap-4 py-6 lg:grid-cols-6">
                    ...
                    <div className="space-y-2">
                        <Label htmlFor="delivery-endpoint">
                            Endpoint contains
                        </Label>
                        <Input
                    ...
                    <div className="space-y-2">
                        <Label htmlFor="delivery-subscription">
                            Subscription ID
                        </Label>
                        <Input
            ...
            <div className="flex flex-1 flex-col rounded-2xl border bg-card shadow-xs">
                <div className="flex flex-wrap items-center justify-between gap-3 border-b px-4 py-3">
                    ...
                    <div className="flex items-center gap-2">
                        <Button
                            ...
                            variant="secondary"
                            disabled={
                                selectedDlq.size === 0 || replayDlq.isPending
                            }
                ...
                <div className="max-h-[60vh] overflow-auto">
                    <table className="min-w-full divide-y text-sm">
                        <thead className="bg-muted/40 text-left text-[11px] text-muted-foreground uppercase">
                            <tr>
                        ...
                        <tbody className="divide-y">
                            ...
                            {!isLoading &&
                                items.map((item) => {
                                    ...
                                    const latencyLabel = item.latencyMs
                                        ? `${item.latencyMs} ms`
                                        : '--';
                                    ...
                                    return (
                                        <tr
                                            key={item.id}
                                            className={cn(
                                                isDlq && 'bg-destructive/5',
                                            )}
                                        >
                                            <td className="px-4 py-4 align-top">
                                                {isDlq ? (
                                                    <Checkbox
                                                        checked={selectedDlq.has(
                                                            item.id,
                                                        )}
                                                        onCheckedChange={(
                                                            checked,
                                                        ...
                                                        <SheetContent className="sm:max-w-2xl">
                                                            ...
                                                            <ScrollArea className="flex-1 space-y-6 px-1 pb-6">
                                                                ...
                                                                <section className="space-y-2">
                                                                    ...
                                                                    <p className="rounded-md border bg-muted/40 p-3 text-sm break-all text-foreground">
                                                                        {item.endpoint ??
                                                                            'N/A'}
                                                                    </p>
                                                                </section>
