...
import { AlertTriangle, ArrowRight, Boxes, ClipboardList, Factory, PackagePlus, Repeat } from 'lucide-react';
...
const MOVEMENT_TYPE_META: Record<MovementType, { label: string; variant: 'default' | 'secondary' | 'outline' | 'destructive' }> = {
...
export function InventoryPage() {
    ...
    const lowStockItems = useMemo(() => lowStockQuery.data?.items ?? [], [lowStockQuery.data?.items]);
    const recentMovements = useMemo(() => movementsQuery.data?.items ?? [], [movementsQuery.data?.items]);
    ...
    const trackedSkuCount = useMemo(() => {
        return resolveMetaNumber(itemsQuery.data?.meta, [['totals', 'skus'], ['totals', 'items'], ['total'], ['count']], items.length);
    }, [itemsQuery.data?.meta, items.length]);

    const lowStockCount = useMemo(() => {
        return resolveMetaNumber(lowStockQuery.data?.meta, [['totals', 'alerts'], ['total'], ['count']], lowStockItems.length);
    ...
    }, [trackedSkuCount, lowStockCount, movementsLast24h, itemsQuery.isLoading, lowStockQuery.isLoading, movementsQuery.isLoading]);
        ...
        <div className="flex flex-1 flex-col gap-6">
            ...
            <div className="flex flex-wrap items-start justify-between gap-4">
                <div className="space-y-1">
                    <p className="text-xs uppercase tracking-wide text-muted-foreground">Supply operations</p>
                    <h1 className="text-2xl font-semibold text-foreground">Inventory control</h1>
                    <p className="text-sm text-muted-foreground">
                        Track on-hand balances, stay ahead of low stock, and keep every adjustment auditable.
                    </p>
                </div>
                <div className="flex flex-wrap gap-2">
                    <Button type="button" variant="outline" size="sm" onClick={() => navigate('/app/inventory/movements/new')}>
                        ...
                        <CardContent className="flex items-center gap-4 p-4">
                            ...
                            <div>
                                ...
                                {stat.loading ? (
                                    ...
                                    <p className="mt-1 text-2xl font-semibold text-foreground">{formatNumber(stat.value)}</p>
                                )}
            ...
            <Card className="border-border/70">
                <CardHeader className="gap-2 pb-2">
                    <CardTitle className="text-base font-semibold">Workspace shortcuts</CardTitle>
                    <p className="text-sm text-muted-foreground">Jump into the tools buyers and site managers use most often.</p>
                </CardHeader>
                <CardContent className="grid gap-3 md:grid-cols-2">
                    {QUICK_ACTIONS.map((action) => (
                        <Link key={action.href} to={action.href} className="group rounded-lg border border-border/70 p-4 transition hover:border-primary">
                ...
                <Card className="border-border/70">
                    <CardHeader className="flex flex-row items-start justify-between gap-4 pb-2">
                        <div>
                            ...
                            <p className="text-sm text-muted-foreground">Prioritize SKUs that need replenishment to protect uptime.</p>
                        </div>
                        <Button asChild variant="ghost" size="sm">
                            <Link to="/app/inventory/alerts/low-stock">View all</Link>
                        </Button>
                    </CardHeader>
                    <CardContent>
                        {lowStockQuery.isLoading ? (
                            ...
                            <div className="divide-y divide-border/60">
                                {lowStockItems.map((alert) => (
                                    <div key={`${alert.itemId}-${alert.locationName ?? 'global'}`} className="flex flex-col gap-2 py-4 md:flex-row md:items-center md:justify-between">
                                        <div>
                                            <Link to={`/app/inventory/items/${alert.itemId}`} className="font-semibold text-primary">
                                                {alert.sku}
                                            </Link>
                                            <p className="text-sm text-muted-foreground">{alert.name}</p>
                                            <p className="text-xs text-muted-foreground">
                                                {alert.locationName ?? 'Multi-site'} · {alert.category ?? 'Uncategorized'}
                                            </p>
                                        </div>
                                        <div className="flex flex-col gap-1 text-sm text-muted-foreground">
                                            <StockBadge onHand={alert.onHand} minStock={alert.minStock} uom={alert.uom ?? undefined} />
                                            {alert.suggestedReorderDate && (
                                                <p className="text-xs">Reorder by {formatDate(alert.suggestedReorderDate, { dateStyle: 'medium' })}</p>
                                            )}
                                        </div>
                                    </div>
                                ))}
                ...
                <Card className="border-border/70">
                    <CardHeader className="flex flex-row items-start justify-between gap-4 pb-2">
                        <div>
                            ...
                            <p className="text-sm text-muted-foreground">Every transaction is audit-logged with source and timestamp.</p>
                        </div>
                        <Button asChild variant="ghost" size="sm">
                        ...
                        {movementsQuery.isLoading ? (
                            ...
                            <div className="space-y-4">
                                {recentMovements.map((movement) => {
                                    const typeMeta = MOVEMENT_TYPE_META[movement.type] ?? { label: movement.type, variant: 'outline' as const };

                                    return (
                                        <div key={movement.id} className="rounded-lg border border-border/60 p-4">
                                            <div className="flex items-start justify-between gap-3">
                                                <div>
                                                    <Link to={`/app/inventory/movements/${movement.id}`} className="font-semibold text-primary">
                                                        {movement.movementNumber}
                                                    </Link>
                                                    <p className="text-sm text-muted-foreground">
                                                        {movement.referenceLabel ?? 'Manual entry'}
                                                    </p>
                                                </div>
                                                <Badge variant={typeMeta.variant}>{typeMeta.label}</Badge>
                                            </div>
                                            <div className="mt-2 flex flex-wrap items-center gap-3 text-xs text-muted-foreground">
                                                <span>{formatDate(movement.movedAt, { dateStyle: 'medium', timeStyle: 'short' })}</span>
                                                {movement.fromLocationName && <span>From {movement.fromLocationName}</span>}
                                                {movement.toLocationName && <span>To {movement.toLocationName}</span>}
                                                <span>{movement.lineCount} line(s)</span>
                                            </div>
                                        </div>
                                    );
                                })}
