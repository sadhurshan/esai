...
import { Boxes, FileSearch2, Filter, PlusCircle, RefreshCcw } from 'lucide-react';
...
const movementTypeOptions: Array<{ label: string; value: MovementFilterState['type'] }> = [
        ...
        () => [
            {
                ...
                title: 'Movement #',
                render: (movement) => (
                    <Link className="font-semibold text-primary" to={`/app/inventory/movements/${movement.id}`}>
                ...
                render: (movement) => (
                    <div className="flex items-center justify-end gap-2">
                        <Button asChild size="sm" variant="ghost">
                            <Link to={`/app/inventory/movements/${movement.id}`}>Open</Link>
                        </Button>
                    </div>
                ),
            },
        ...
        <div className="flex flex-1 flex-col gap-6">
            ...
            <div className="flex flex-wrap items-start justify-between gap-4">
                <div className="space-y-1">
                    ...
                    <p className="text-sm text-muted-foreground">Trace every receipt, issue, transfer, and adjustment.</p>
                </div>
                <Button type="button" size="sm" onClick={() => navigate('/app/inventory/movements/new')}>
            ...
            <Card className="border-border/70">
                <CardContent className="grid gap-4 py-6 md:grid-cols-5">
                    <div className="space-y-2">
                        ...
                        <Select value={filters.type} onValueChange={(value) => handleFilterChange('type', value as MovementFilterState['type'])}>
                            <SelectTrigger>
                                    ...
                                    itemOptions.map((item) => (
                                        <SelectItem key={item.id} value={item.id}>
                                            <div className="flex flex-col text-left">
                                                <span className="text-sm font-medium leading-tight">{item.sku}</span>
                    ...
                    <div className="space-y-2">
                                ...
                                {locationsQuery.isLoading ? (
                                    ...
                                    locationOptions.map((location) => (
                                        <SelectItem key={location.id} value={location.id}>
                                            <div className="flex flex-col text-left">
                                                <span className="text-sm font-medium leading-tight">{location.name}</span>
                                                {location.siteName ? (
                                                    <span className="text-xs text-muted-foreground">{location.siteName}</span>
                                                ) : null}
                                            </div>
                                        </SelectItem>
                                    ))
                        ...
                        <p className="text-xs text-muted-foreground">Pick a bin/site to scope history.</p>
                    </div>
                    <div className="space-y-2">
                        ...
                        <Input
                            ...
                            onChange={(event: ChangeEvent<HTMLInputElement>) => handleFilterChange('dateFrom', event.target.value)}
                        />
                    </div>
                    <div className="space-y-2">
                        <Label>Date to</Label>
                        <Input
                            ...
                            onChange={(event: ChangeEvent<HTMLInputElement>) => handleFilterChange('dateTo', event.target.value)}
                        />
                    </div>
                    <div className="md:col-span-5 flex flex-wrap items-center gap-2 text-xs text-muted-foreground">
                        ...
                        <div className="inline-flex items-center gap-2 rounded-full bg-muted/60 px-3 py-1 text-[11px] uppercase tracking-wide">
            ...
            <DataTable
                ...
                emptyState={
                    <EmptyState
                        ...
                        icon={<FileSearch2 className="h-10 w-10 text-muted-foreground" />}
                        ctaLabel="Post movement"
                        ctaProps={{ onClick: () => navigate('/app/inventory/movements/new') }}
                    />
                }
            />
            ...
            <div className="flex flex-wrap items-center justify-between gap-3">
                ...
                <div className="flex items-center gap-2">
                    ...
                    <Button type="button" variant="outline" size="sm" disabled={!nextCursor} onClick={() => setCursor(nextCursor)}>
                        Next
                    </Button>
                </div>
            </div>
