...
const REVIEW_DIALOG_COPY: Record<
    ReviewActionType,
    {
        title: string;
        description: string;
...
> = {
    approve: {
        ...
        description:
            'Approve the invoice and move it forward to Accounts Payable.',
...
export function InvoiceDetailPage() {
    ...
    const [activeAction, setActiveAction] = useState<ReviewActionType | null>(
        null,
    );
    ...
    const receivingEntries = useMemo(
        () => (receivingEnabled && grnsQuery.data ? grnsQuery.data.items : []),
        [receivingEnabled, grnsQuery.data],
    );
        ...
        () =>
            invoice
                ? buildInvoiceReviewTimeline(invoice, {
                      formatDate,
                      events: poEvents,
                      receiving: receivingEntries,
                  })
                : [],
        [formatDate, invoice, poEvents, receivingEntries],
    );
    ...
    const noteRequired =
        activeAction === 'reject' || activeAction === 'requestChanges';
    const reviewDialogCopy = activeAction
        ? REVIEW_DIALOG_COPY[activeAction]
        : null;
    ...
    const dialogDescription =
        reviewDialogCopy?.description ?? 'Share context with the supplier.';
            ...
            <div className="flex flex-wrap items-center justify-between gap-4">
                ...
                <div className="flex flex-wrap items-center gap-2">
                    {invoice.purchaseOrderId ? (
                        ...
                        >
                            <Link
                                to={`/app/purchase-orders/${invoice.purchaseOrderId}`}
                            >
                                PO{' '}
                                {invoice.purchaseOrder?.poNumber ??
                                    `#${invoice.purchaseOrderId}`}
                    ...
                    <CardContent className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                        <div>
                            <p className="text-sm font-semibold text-amber-900">
                                Supplier submission awaiting review
                            </p>
                            <p className="text-sm text-amber-800">
                                ...
                                · share a note so suppliers know what to fix.
                            </p>
                        </div>
                        <div className="flex flex-wrap gap-2">
                            ...
                            <Button
                                ...
                                variant="outline"
                                onClick={() =>
                                    handleOpenDialog('requestChanges')
                                }
                                disabled={isReviewPending}
                            >
                                Request changes
                ...
                <Card className="border-border/70 lg:col-span-2">
                    ...
                    <CardContent className="space-y-4 text-sm">
                        <div className="grid gap-4 md:grid-cols-3">
                            ...
                            <MetadataItem
                                label="Supplier org"
                                value={invoice.supplierCompany?.name ?? '—'}
                            />
                            <MetadataItem
                                label="Invoice date"
                    ...
                    <CardContent>
                        {invoice.matchSummary ? (
                            <dl className="space-y-2 text-sm">
                                {Object.entries(invoice.matchSummary).map(
                                    ([key, value]) => (
                                        <div
                                            key={key}
            ...
            <div className="grid gap-4 lg:grid-cols-3">
                ...
                <Card className="border-border/70">
                    <CardHeader>
                        ...
                        <CardDescription>
                            Combines review events, PO activity, and receiving
                            milestones.
                        </CardDescription>
                    </CardHeader>
                    <CardContent>
            ...
            <Dialog open={dialogOpen} onOpenChange={handleDialogToggle}>
                <DialogContent className="sm:max-w-lg">
                    ...
                    <div className="space-y-2">
                        ...
                        {noteRequired ? (
                            <p className="text-xs text-muted-foreground">
                                A note is required when rejecting or requesting
                                changes.
                            </p>
                        ) : (
                            <p className="text-xs text-muted-foreground">
                                Optional note that is shared with the supplier.
                            </p>
                        )}
                    </div>
                    <DialogFooter>
