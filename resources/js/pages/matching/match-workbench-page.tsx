...
export const MatchWorkbenchPage = () => {
    ...
    const candidates = useMemo(
        () => matchCandidatesQuery.data?.items ?? [],
        [matchCandidatesQuery.data?.items],
    );
    const cursorMeta = (matchCandidatesQuery.data?.meta ??
        null) as CursorMeta | null;
    const nextCursor =
    ...
    const selectedCandidate: MatchCandidate | undefined = candidates.find(
        (candidate) => candidate.id === activeSelectedId,
    );
            ...
            <div className="flex flex-wrap items-start justify-between gap-4">
                <div className="space-y-1">
                    ...
                    <h1 className="text-2xl font-semibold text-foreground">
                        3-way match workbench
                    </h1>
                    <p className="text-sm text-muted-foreground">
                        Reconcile invoices, POs, and receipts with a guided
                        review queue.
                    </p>
                </div>
            </div>
                        ...
                        <div className="flex items-center justify-between">
                            ...
                            >
                                {matchCandidatesQuery.isLoading
                                    ? '…'
                                    : `${formatNumber(candidates.length, { maximumFractionDigits: 0 })} items`}
                            </Badge>
                        </div>
                        <Separator />
                                ...
                                >
                                    <div className="flex flex-wrap items-center justify-between gap-2">
                                        ...
                                        <Badge
                                            ...
                                            className={cn(
                                                'text-xs capitalize',
                                                statusToClass(candidate.status),
                                            )}
                                        >
                                            {candidate.status}
                                        </Badge>
