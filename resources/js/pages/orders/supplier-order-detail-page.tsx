...
import type { CreateShipmentPayload, SalesOrderDetail, SalesOrderShipment } from '@/types/orders';
...
export function SupplierOrderDetailPage() {
    ...
    const supplierOrderQuery = useSupplierOrder(orderId ?? null, { enabled: Boolean(orderId) });
    ...
    const [statusDialog, setStatusDialog] = useState<{ shipmentId: number; status: 'in_transit' | 'delivered' } | null>(null);
    const [deliveredAtInput, setDeliveredAtInput] = useState(() => new Date().toISOString().slice(0, 16));
        ...
        return (
            <div className="flex flex-1 flex-col gap-6">
                ...
                <EmptyState
                    ...
                    ctaProps={{ onClick: () => navigate('/app/supplier/orders') }}
                />
            </div>
        );
    }
    ...
    const canCreateShipment = hasFulfillableLines(order) && ['accepted', 'partially_fulfilled'].includes(order.status);
        ...
        updateShipmentStatusMutation.mutate(
            {
                ...
                deliveredAt: statusDialog.status === 'delivered' ? deliveredAtInput : undefined,
            },
            {
                onSuccess: () => {
        ...
        <div className="flex flex-1 flex-col gap-6">
            ...
            <Card className="border-border/70">
                <CardHeader className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                    <div className="space-y-1">
                        ...
                        <CardDescription>
                            Linked to PO #{order.poId}. Keep the buyer posted on fulfilment and shipment events.
                        </CardDescription>
                        <div className="flex flex-wrap gap-2 pt-2 text-sm text-muted-foreground">
                            <Badge variant="outline" className="border-primary/50 text-primary">
                                Buyer: {order.buyerCompanyName ?? `#${order.buyerCompanyId}`}
                            </Badge>
                            <Badge variant="outline">Currency: {order.currency}</Badge>
                ...
                <CardContent className="space-y-6">
                    ...
                    {canAcknowledge ? (
                        <Alert className="border-amber-200 bg-amber-50">
                            <AlertTitle className="text-amber-900">Awaiting acknowledgement</AlertTitle>
                            <AlertDescription className="text-amber-900/80">
                                Accept or decline this order so the buyer knows if you can fulfil it.
                            </AlertDescription>
                            <div className="mt-4 flex flex-wrap gap-3">
                                <Button type="button" onClick={() => handleAck('accept')} disabled={ackMutation.isPending}>
                        ...
                        <Alert className="border-muted-foreground/40">
                            <AlertTitle>
                                Order {acknowledgement.decision === 'accept' ? 'accepted' : 'declined'} on{' '}
                                {formatDate(acknowledgement.acknowledgedAt)}
                            </AlertTitle>
                            {acknowledgement.reason ? (
                                <AlertDescription>{acknowledgement.reason}</AlertDescription>
                            ) : null}
                        </Alert>
                    ) : null}
                </CardContent>
            ...
            <div className="grid gap-6 lg:grid-cols-2">
                <Card className="border-border/70">
                    <CardHeader>
                        ...
                        <CardDescription>Reference the buyer preferences before dispatch.</CardDescription>
                    </CardHeader>
                    <CardContent>
                ...
                <Card className="border-border/70">
                    <CardHeader>
                        <CardTitle className="text-lg font-semibold">Linked documents</CardTitle>
                        <CardDescription>Jump over to the connected purchase order if you need revisions.</CardDescription>
                    </CardHeader>
                    <CardContent className="flex flex-wrap gap-3 text-sm">
                        <Button asChild variant="outline" size="sm">
                            <Link to={`/app/suppliers/pos/${order.poId}`} className="gap-2">
                                <Package className="h-4 w-4" /> Supplier PO view
                            </Link>
                        </Button>
                        <Button asChild variant="ghost" size="sm">
                            <Link to={`/app/purchase-orders/${order.poId}`} className="gap-2">
                                <ExternalLink className="h-4 w-4" /> Buyer PO detail
                            </Link>
                        </Button>
                    </CardContent>
                </Card>
                    ...
                    <Card className="border-border/70">
                        <CardHeader className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
                            <div>
                                ...
                                <CardDescription>Log consignments as they leave the dock.</CardDescription>
                            </div>
                            <Button
                                type="button"
                                onClick={() => setShipmentDialogOpen(true)}
                                disabled={!canCreateShipment || createShipmentMutation.isPending}
                            >
                                <PackagePlus className="mr-2 h-4 w-4" /> Create shipment
                            </Button>
                        </CardHeader>
                                        ...
                                        <div className="flex flex-wrap items-start justify-between gap-3">
                                            <div>
                                                ...
                                                {shipment.trackingNumber ? (
                                                    ...
                                                    <span className="text-sm text-muted-foreground">Tracking pending</span>
                                                )}
                                            </div>
                                        ...
                                        <div className="mt-4 flex flex-wrap gap-2">
                                            {shipment.status === 'pending' ? (
                                                <Button
                                                    ...
                                                    variant="secondary"
                                                    size="sm"
                                                    onClick={() => handleMarkInTransit(shipment)}
                                                    disabled={updateShipmentStatusMutation.isPending}
                                                >
                                                    <Repeat2 className="mr-2 h-4 w-4" /> Mark in transit
                                                </Button>
                                            ) : null}
                ...
                <TabsContent value="timeline">
                    <Card className="border-border/70">
                        <CardHeader>
                            ...
                            <CardDescription>Status changes, acknowledgements, and shipment events.</CardDescription>
                        </CardHeader>
                        <CardContent>
                            <OrderTimeline
            ...
            <Dialog open={declineDialogOpen} onOpenChange={setDeclineDialogOpen}>
                <DialogContent>
                    <DialogHeader>
                        ...
                        <DialogDescription>
                            Share a short message so the buyer knows why this cannot be fulfilled.
                        </DialogDescription>
                    </DialogHeader>
                    <Textarea
                        rows={4}
            ...
            <Dialog open={Boolean(statusDialog)} onOpenChange={(open) => !open && setStatusDialog(null)}>
                <DialogContent>
                    <DialogHeader>
                        <DialogTitle>
                            {statusDialog?.status === 'delivered' ? 'Mark shipment delivered' : 'Mark shipment in transit'}
                        </DialogTitle>
                        <DialogDescription>
                            Status updates notify the buyer instantly and keep downstream receiving accurate.
                        </DialogDescription>
                    </DialogHeader>
                    {statusDialog?.status === 'delivered' ? (
                        <div className="space-y-2">
                            <label className="text-xs font-medium uppercase text-muted-foreground" htmlFor="delivered-at-input">
                                Delivery timestamp
                            </label>
                            <Input
                                ...
                                onChange={(event) => setDeliveredAtInput(event.target.value)}
                            />
                        </div>
                    ) : null}
                    <DialogFooter>
                        <Button type="button" variant="outline" onClick={() => setStatusDialog(null)} disabled={updateShipmentStatusMutation.isPending}>
                            Cancel
                        </Button>
                        <Button type="button" onClick={handleStatusDialogSubmit} disabled={updateShipmentStatusMutation.isPending}>
                            {statusDialog?.status === 'delivered' ? 'Confirm delivery' : 'Confirm in transit'}
                        </Button>
                    </DialogFooter>
                </DialogContent>
            </Dialog>
