...
const optionalMoqSchema = z
    ...
    .refine((value) => value === '' || !Number.isNaN(Number(value)), 'Minimum order must be numeric.')
    .refine((value) => value === '' || Number.isInteger(Number(value)), 'Minimum order must be a whole number.')
    .refine((value) => value === '' || Number(value) >= 1, 'Minimum order must be at least 1.');
...
const taxCodeIdSchema = z
    ...
    .refine((value) => value === '' || /^[0-9]+$/.test(value), 'Select a valid tax code.');
...
const documentAttachmentSchema = z
    .object({
        id: z
            ...
            .refine((value) => Number.isFinite(value) && value > 0, 'Attachment id is invalid.'),
        filename: z.string().min(1, 'Attachment filename is required.').default('Attachment'),
    ...
    .transform((value) => ({
        ...
        sizeBytes: value.sizeBytes !== undefined ? Number(value.sizeBytes) || 0 : 0,
