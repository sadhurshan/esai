...
import { PackagePlus, PackageSearch, Send } from 'lucide-react';
...
import {
    GrnLineEditor,
    type GrnLineFormValue,
} from '@/components/receiving/grn-line-editor';
...
const grnLineSchema = z
    .object({
        ...
        notes: z
            ...
            .max(
                MAX_LINE_NOTE_LEN,
                `Notes cannot exceed ${MAX_LINE_NOTE_LEN} characters`,
            )
            .nullable()
            .optional(),
    })
...
const grnFormSchema = z.object({
    ...
    reference: z
        ...
        .max(
            MAX_REFERENCE_LEN,
            `Reference cannot exceed ${MAX_REFERENCE_LEN} characters`,
        )
        .optional()
        .or(z.literal('')),
    notes: z
...
export function ReceivingCreatePage() {
    ...
    const [selectedPoSummary, setSelectedPoSummary] =
    ...
    const isSupplierPersona =
        activePersona?.type === 'supplier' || supplierRole;
    ...
    const rootLinesError =
        ...
        !Array.isArray(formState.errors.lines) &&
        formState.errors.lines.message
            ? formState.errors.lines.message
            : null;

    const effectivePoId =
        Number.isFinite(activePoId) && !isSupplierPersona ? activePoId : 0;
    ...
    const submitGrn = async (
        ...
        if (!Array.isArray(values.lines) || values.lines.length === 0) {
            setError('lines', {
                type: 'manual',
                message:
                    'This purchase order has no open quantities left to receive. Choose another PO to continue.',
            });
            return;
        }
        ...
        if (!values.lines.some((line) => line.qtyReceived > 0)) {
            setError('lines', {
                type: 'manual',
                message:
                    'Enter at least one received quantity greater than zero before saving.',
            });
            return;
        }
    ...
    const activePo =
        selectedPoSummary ??
        (poDetail as PurchaseOrderSummary | undefined) ??
        null;
        ...
        <div className="flex flex-1 flex-col gap-6">
            ...
            <div className="flex flex-wrap items-start justify-between gap-4">
                <div className="space-y-1">
                    ...
                    <p className="text-sm text-muted-foreground">
                        Select a purchase order, capture arriving quantities,
                        and keep receiving reconciled.
                    </p>
                </div>
                <Button
                    type="button"
            ...
            {mutationError ? (
                <Alert variant="destructive">
                    ...
                    <AlertDescription>
                        {mutationError.message ?? 'Please try again.'}
                    </AlertDescription>
                </Alert>
            ) : null}
            ...
            <Card className="border-border/70">
                <CardHeader>
                    ...
                    <CardDescription>
                        GRNs must reference an existing PO to maintain
                        traceability.
                    </CardDescription>
                </CardHeader>
                <CardContent className="space-y-4">
                    {activePo ? (
                        ...
                        <div className="rounded-md border border-dashed border-border/70 bg-muted/30 p-4 text-sm text-muted-foreground">
                            ...
                            <p className="mt-1 text-sm text-muted-foreground">
                                Choose a PO to load open lines and available
                                quantities.
                            </p>
                            <Button
                                type="button"
                    ...
                    {activePo ? (
                        <div className="grid gap-4 md:grid-cols-3">
                            ...
                            />
                            <MoneyCell
                                amountMinor={activePo.taxAmountMinor}
                                currency={activePo.currency}
                                label="Tax"
                            />
                            <MoneyCell
                                amountMinor={activePo.totalMinor}
                                currency={activePo.currency}
                                label="Total"
                            />
                        </div>
                ...
                <CardContent className="grid gap-4 md:grid-cols-2">
                    <div className="space-y-2">
                        ...
                        {formState.errors.receivedAt ? (
                            <p className="text-xs text-destructive">
                                {formState.errors.receivedAt.message}
                            </p>
                        ) : null}
                    </div>
                    <div className="space-y-2">
                    ...
                    <div className="space-y-2 md:col-span-2">
                        ...
                        <p className="text-xs text-muted-foreground">
                            Capture damages, shortages, or carrier issues.
                            Attachments are uploaded after saving the draft.
                        </p>
                    </div>
                </CardContent>
            </Card>
            ...
            {rootLinesError ? (
                <p className="text-sm text-destructive">{rootLinesError}</p>
            ) : null}
            ...
            <div className="sticky bottom-0 flex flex-col gap-3 rounded-lg border border-border/70 bg-card/95 p-4 shadow-sm">
                <div className="flex flex-wrap items-center justify-between gap-3">
                    <div className="text-sm text-muted-foreground">
                        Posting will update PO received quantities and unlock
                        matching.
                    </div>
...
function mapPoLine(line: PurchaseOrderLine): GrnLineFormValue {
    ...
    const remaining =
        line.remainingQuantity ?? Math.max(ordered - previously, 0);
...
function PurchaseOrderPickerDialog({
    open,
    onOpenChange,
    onSelect,
        ...
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent className="max-w-2xl">
                <DialogHeader>
                    ...
                    <DialogDescription>
                        Search approved purchase orders to base your goods
                        receipt on.
                    </DialogDescription>
                </DialogHeader>
                ...
                <div className="space-y-4">
                    ...
                    {poQuery.isLoading ? (
                        <div className="flex items-center gap-2 text-sm text-muted-foreground">
                            <Spinner className="h-4 w-4" /> Loading purchase
                            orders…
                        </div>
                    ) : null}
                    ...
                    {!poQuery.isLoading && items.length === 0 ? (
                        <p className="text-sm text-muted-foreground">
                            No purchase orders matched your search.
                        </p>
                    ) : null}
