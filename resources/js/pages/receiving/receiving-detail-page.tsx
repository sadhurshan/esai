...
export function ReceivingDetailPage() {
    ...
    const linesWithNcrs = grn.lines.filter(
        (line) => (line.ncrs?.length ?? 0) > 0 || line.ncrFlag,
    ).length;
    const ncrSummary = {
            ...
            <div className="flex flex-wrap items-start justify-between gap-4">
                    ...
                    <p className="text-sm text-muted-foreground">
                        ...
                        {grn.purchaseOrderNumber ? (
                            <Link
                                ...
                                to={`/app/purchase-orders/${grn.purchaseOrderId}`}
                            >
                                {grn.purchaseOrderNumber}
                            </Link>
                        ) : (
                ...
                <div className="flex flex-wrap gap-2">
                    ...
                    <ExportButtons
                        ...
                        documentId={grn.id}
                        reference={
                            grn.grnNumber ? `GRN ${grn.grnNumber}` : undefined
                        }
                        size="sm"
                    />
                ...
                <Card className="border-border/70">
                    ...
                    <CardContent>
                        {grn.timeline && grn.timeline.length > 0 ? (
                            ...
                            <p className="text-sm text-muted-foreground">
                                No timeline entries captured yet.
                            </p>
                        )}
                    </CardContent>
                </Card>
                ...
                <CardContent className="space-y-4 text-sm">
                    {ncrSummary.total === 0 ? (
                        <p className="text-muted-foreground">
                            No NCRs recorded for this receipt.
                        </p>
                    ) : (
                        <div className="grid gap-4 sm:grid-cols-3">
                            <QualityMetric
                                label="Open NCRs"
                                value={ncrSummary.open}
                                highlight={ncrSummary.open > 0}
                            />
                            <QualityMetric
                                label="Total NCRs"
                                value={ncrSummary.total}
                            />
                            <QualityMetric
                                label="Lines flagged"
                                value={ncrSummary.flaggedLines}
                            />
                        </div>
                    )}
                </CardContent>
                    ...
                    <FileDropzone
                        ...
                        description={
                            attachMutation.isPending
                                ? 'Uploading…'
                                : 'PDF or image files up to 25 MB.'
                        }
    ...
    return (
        <tr className="border-t border-border/60">
            <td className="py-3 pr-4">
                <p className="font-medium text-foreground">
                    {line.description ?? `PO line #${line.poLineId}`}
                </p>
                <p className="text-xs text-muted-foreground">
            ...
            <td className="py-3 pr-4">
                {formatQuantity(line.orderedQty, line.uom, formatNumber)}
            </td>
            <td className="py-3 pr-4">
                {formatQuantity(
            ...
            <td className="py-3 pr-4 font-semibold text-foreground">
                {formatQuantity(line.qtyReceived, line.uom, formatNumber)}
            </td>
            ...
            <td className="py-3 pr-4">
                <div className="space-y-2">
                    {ncrs.length === 0 ? (
                        <p className="text-xs text-muted-foreground">
                            {isFlagged ? 'Flagged for review' : 'No NCRs'}
                        </p>
                    ) : (
                        <div className="space-y-2">
                            {ncrs.map((ncr) => (
                                ...
                                >
                                    <div>
                                        <span
                                            className={`font-semibold ${ncr.status === 'open' ? 'text-destructive' : 'text-muted-foreground'}`}
                                        >
                                            NCR #{ncr.id}
                                        </span>
...
function formatQuantity(
    ...
    const formatted = formatNumber(value, {
        maximumFractionDigits: Math.abs(value) >= 1 ? 2 : 3,
    });
        ...
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent>
                <DialogHeader>
                    ...
                    <DialogDescription>
                        {line
                            ? `Line ${line.lineNo ?? line.poLineId} · ${line.description ?? 'PO line'}`
