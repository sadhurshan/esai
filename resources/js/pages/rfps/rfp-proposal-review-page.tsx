...
export function RfpProposalReviewPage() {
    ...
    const rfpsEnabled = hasFeature('rfps_enabled') || hasFeature('projects_enabled') || hasFeature('sourcing_access');
        ...
        return (
            <div className="flex flex-1 flex-col gap-6">
                ...
                <EmptyState
                    ...
                    description={rfpQuery.error?.message ?? 'This project is unavailable or you lack access.'}
        ...
        return (
            <div className="flex flex-1 flex-col gap-6">
                ...
                <EmptyState
                    ...
                    description={proposalsQuery.error?.message ?? 'Please retry or refresh the page.'}
                    icon={<AlertTriangle className="h-10 w-10 text-destructive" />}
                    ctaLabel="Retry"
            ...
            <div className="flex flex-wrap items-center justify-between gap-4">
                <div>
                    ...
                    <p className="text-sm text-muted-foreground">
                        Evaluate {proposalCount === 1 ? 'the latest proposal' : `${proposalCount} proposals`} for this project.
                    </p>
                </div>
            ...
            <div className="grid gap-6 lg:grid-cols-[0.85fr_1.15fr]">
                <Card className="border-border/70">
                    ...
                    <CardContent className="space-y-4 text-sm">
                        <DetailPair label="Problem & objectives" value={rfp.problemObjectives} />
                        <DetailPair label="Scope" value={rfp.scope} />
                        <DetailPair label="Evaluation criteria" value={rfp.evaluationCriteria} />
                        <DetailPair label="Proposal format" value={rfp.proposalFormat} />
                        <div className="grid gap-3 sm:grid-cols-2">
                            <DetailPair label="Published" value={formatDate(rfp.publishedAt)} />
                            <DetailPair label="Last updated" value={formatDate(rfp.updatedAt)} />
                        </div>
                    </CardContent>
                </Card>
                ...
                <Card className="border-border/70">
                    ...
                    <CardContent className="grid gap-4 md:grid-cols-2">
                        <HighlightItem label="Total proposals" value={proposalCount.toString()} />
                        <HighlightItem
                            label="Best price"
                            value={summary?.minPriceMinor == null ? '—' : undefined}
                            money={summary?.minPriceMinor}
                            currency={summary?.currency ?? proposals[0]?.currency ?? 'USD'}
                        />
                        <HighlightItem
                            label="Fastest lead time"
                            value={summary?.minLeadTimeDays == null ? '—' : `${summary.minLeadTimeDays} days`}
                        />
                        <HighlightItem
                            label="Slowest lead time"
                            value={summary?.maxLeadTimeDays == null ? '—' : `${summary.maxLeadTimeDays} days`}
                        />
                    </CardContent>
                </Card>
            </div>
                                ...
                                {proposals.map((proposal) => (
                                    <tr
                                        ...
                                        className={cn(
                                            ...
                                            proposal.id === highlightPriceId && 'bg-emerald-50/60',
                                            proposal.id === highlightLeadId && 'ring-1 ring-blue-200',
                                        )}
                                    >
                                        <td className="py-3 pr-4 text-foreground">
                                            <p className="font-semibold">{proposal.supplierCompany?.name ?? 'Supplier'}</p>
                                            <p className="text-xs text-muted-foreground">#{proposal.supplierCompanyId ?? '—'}</p>
                                        </td>
                                        <td className="py-3 pr-4">
                                            {proposal.priceTotalMinor ? (
                                                <MoneyCell
                                                    amountMinor={proposal.priceTotalMinor}
                                        ...
                                        <td className="py-3 pr-4">
                                            {proposal.leadTimeDays ? (
                                                <span className="font-semibold">{proposal.leadTimeDays} days</span>
                                            ) : (
                                                <span className="text-muted-foreground">—</span>
                                            )}
                                        </td>
                                        <td className="py-3 pr-4 text-muted-foreground">
                                            <p className="line-clamp-3">{proposal.approachSummary ?? 'No summary provided'}</p>
                                        </td>
                                        <td className="py-3 pr-4">
                                            <Badge variant="secondary">{proposal.attachmentsCount ?? 0}</Badge>
                                        </td>
                                        <td className="py-3 text-muted-foreground">{formatDate(proposal.createdAt)}</td>
                                    </tr>
                                ))}
                            </tbody>
                        </table>
...
function determineLowestPriceProposal(proposals: RfpProposalSummary[]): number | null {
    let minValue = Number.POSITIVE_INFINITY;
    let proposalId: number | null = null;

    proposals.forEach((proposal) => {
        if (typeof proposal.priceTotalMinor === 'number' && proposal.priceTotalMinor < minValue) {
...
function determineFastestProposal(proposals: RfpProposalSummary[]): number | null {
    let minValue = Number.POSITIVE_INFINITY;
    let proposalId: number | null = null;

    proposals.forEach((proposal) => {
        if (typeof proposal.leadTimeDays === 'number' && proposal.leadTimeDays < minValue) {
...
function DetailPair({ label, value }: { label: string; value?: string | null }) {
    return (
        <div>
            ...
            <p className="whitespace-pre-line text-sm font-medium text-foreground">{value && value.length ? value : '—'}</p>
        </div>
    );
}
...
function HighlightItem({
    ...
    return (
        <div className="rounded-lg border border-border/60 p-4">
            <p className="text-xs uppercase tracking-wide text-muted-foreground">{label}</p>
            {typeof money === 'number' ? (
                <MoneyCell amountMinor={money} currency={currency ?? 'USD'} className="text-base font-semibold" />
            ) : (
            ...
            <div className="grid gap-6 lg:grid-cols-[0.85fr_1.15fr]">
                ...
                <Card className="border-border/70">
                    <CardContent className="grid gap-4 p-6 md:grid-cols-2">
                        {Array.from({ length: 4 }).map((_, index) => (
                            <Skeleton key={`highlight-${index}`} className="h-16 w-full" />
                        ))}
                    </CardContent>
                </Card>
            </div>
