        ...
        <div className="flex flex-1 flex-col gap-6">
            ...
            <div className="flex flex-wrap items-center gap-3 rounded-xl border bg-card px-4 py-3 shadow-sm">
                ...
                <div className="flex items-center gap-2">
                    <Select
                        value={statusFilter}
                        onValueChange={(value) =>
                            handleStatusChange(value as RfqStatusFilter)
                        }
                    >
                ...
                <div className="flex items-center gap-2">
                    <Select
                        value={biddingFilter}
                        onValueChange={(value) =>
                            setBiddingFilter(value as BiddingFilter)
                        }
                    >
            ...
            {rfqsQuery.isError ? (
                <Alert variant="destructive">
                    ...
                    <AlertDescription>
                        We ran into an issue retrieving RFQs. Please retry in a
                        ...
                        {showSkeleton
                            ...
                            : items.map((rfq) => {
                                  const statusPresentation =
                                      getStatusPresentation(rfq.status);
                                  const extended = rfq as Rfq & {
                                      title?: string | null;
                                  };
                                  const derivedTitle =
                                      ...
                                      extended.title.trim().length > 0
                                          ...
                                          : rfq.itemName;
                                  const quantityTotal =
                                      resolveQuantityTotal(rfq);
                                  const methodLabel = getRfqMethodLabel(
                                      (rfq as { method?: string | null })
                                          .method ?? undefined,
                                  );
                                  const materialLabel =
                                      (rfq as { material?: string | null })
                                          .material ?? '—';
                                  const publishAt =
                                      (rfq as { publishAt?: string | null })
                                          .publishAt ??
                                      (rfq as { sentAt?: string | null })
                                          .sentAt;
                                  const dueAt =
                                      (rfq as { dueAt?: string | null })
                                          .dueAt ??
                                      (rfq as { deadlineAt?: string | null })
                                          .deadlineAt;
