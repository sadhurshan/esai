...
import { Shield, ShieldQuestion, ShieldAlert, RefreshCcw } from 'lucide-react';
...
const LEVEL_BADGE_VARIANTS: Record<string, 'default' | 'secondary' | 'outline' | 'destructive'> = {
    read: 'secondary',
    write: 'default',
    admin: 'destructive',
};
...
export function CompanyRolesPage() {
    ...
    const roles = useMemo(() => roleTemplatesQuery.data?.roles ?? [], [roleTemplatesQuery.data]);
    const permissionGroups = useMemo(() => roleTemplatesQuery.data?.permissionGroups ?? [], [roleTemplatesQuery.data]);
        ...
        <div className="space-y-6">
            ...
            <div>
                ...
                <p className="text-sm text-muted-foreground">
                    Review the permissions each seeded role template includes before inviting new teammates.
                </p>
            </div>
            <Alert>
                ...
                <AlertDescription>
                    Use role descriptions and permission scopes below when deciding which access level to grant. Editing templates is limited to platform operators.
                </AlertDescription>
            </Alert>
            <div className="grid gap-4 sm:grid-cols-2">
                <Card>
                    <CardContent className="space-y-1 p-4">
                        <p className="text-sm text-muted-foreground">Seeded roles</p>
                        <p className="text-2xl font-semibold">{stats.roleCount}</p>
                        <p className="text-xs text-muted-foreground">Owner, buyer, supplier, and finance templates</p>
                    </CardContent>
                </Card>
                <Card>
                    <CardContent className="space-y-1 p-4">
                        <p className="text-sm text-muted-foreground">Unique permissions</p>
                        <p className="text-2xl font-semibold">{stats.permissionCount}</p>
                        <p className="text-xs text-muted-foreground">Across sourcing, suppliers, orders, billing, analytics, and workspace</p>
                    </CardContent>
                </Card>
            </div>
            <div className="flex flex-wrap items-center gap-3">
                ...
                {roleTemplatesQuery.isFetching ? <span className="text-xs text-muted-foreground">Updating…</span> : null}
            </div>
            <div className="grid gap-6 lg:grid-cols-[1.4fr_1fr]">
                <Card>
                    <CardHeader>
                        <CardTitle>Role catalog</CardTitle>
                        <CardDescription>Select a role to review its permissions.</CardDescription>
                    </CardHeader>
                    <CardContent>
                        {roleTemplatesQuery.isLoading ? (
                            ...
                            <Alert variant="destructive">
                                <AlertTitle>Unable to load role templates</AlertTitle>
                                <AlertDescription>{roleTemplatesQuery.error?.message ?? 'Please try again later.'}</AlertDescription>
                            </Alert>
                        ) : !roles.length ? (
                            <EmptyState
                                ...
                                <div className="space-y-3">
                                    {roles.map((role) => (
                                        <RoleListItem
                                            key={role.slug}
                                            role={role}
                                            isActive={effectiveSelectedRole === role.slug}
                                            onSelect={() => setSelectedRole(role.slug)}
                                        />
                                    ))}
                                </div>
                ...
                <Card>
                    <CardHeader>
                        ...
                        <CardDescription>See how permissions map to feature domains.</CardDescription>
                    </CardHeader>
                                ...
                                <div key={group.id} className="rounded-lg border p-4">
                                    ...
                                    <ul className="mt-3 space-y-2">
                                        {group.permissions.map((permission) => (
                                            <li key={permission.key} className="rounded-md bg-muted/40 p-2">
                                                <div className="flex items-center justify-between gap-2">
                                                    <p className="text-sm font-medium">{permission.label}</p>
                                                    {permission.level ? (
                                                        <Badge variant={LEVEL_BADGE_VARIANTS[permission.level] ?? 'secondary'}>
                                                            {permission.level}
                                                        </Badge>
                                                    ) : null}
                                                </div>
                                                <p className="text-xs text-muted-foreground">{permission.description ?? 'No description provided.'}</p>
                                            </li>
                                        ))}
                                    </ul>
                                </div>
...
interface RolePermissionPanelProps {
    role: RoleTemplate;
    permissionLookup: Map<string, PermissionDefinition & { groupLabel?: string | null }>;
}

function RolePermissionPanel({ role, permissionLookup }: RolePermissionPanelProps) {
    ...
    if (!permissions.length) {
        return (
            <div className="rounded-lg border p-4">
                ...
                <p className="text-xs text-muted-foreground">This template currently has an empty permission set.</p>
            </div>
        );
    }
    ...
    return (
        <div className="space-y-3">
            <div className="rounded-lg border p-4">
                ...
                <p className="text-xs text-muted-foreground">{role.permissions.length} total scopes</p>
            </div>
            <div className="space-y-2 max-h-[32rem] overflow-y-auto pr-1">
                {permissions.map((permissionKey) => {
                    ...
                    const levelVariant = LEVEL_BADGE_VARIANTS[level] ?? 'outline';

                    return (
                        <div key={permissionKey} className="rounded-lg border p-3">
                            <div className="flex items-center justify-between gap-2">
                                <div>
                                    <p className="text-sm font-semibold">{label}</p>
                                    <p className="text-xs text-muted-foreground">{groupLabel}</p>
                                </div>
                                <Badge variant={levelVariant} className="capitalize">
                                    {level}
                                </Badge>
                            </div>
