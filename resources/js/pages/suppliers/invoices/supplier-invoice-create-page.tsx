...
import { AlertTriangle, BadgeDollarSign, Loader2, Wallet } from 'lucide-react';
...
import { useSupplierCreateInvoice, type SupplierInvoiceLineInput } from '@/hooks/api/invoices/use-supplier-create-invoice';
...
export function SupplierInvoiceCreatePage() {
    ...
    const [poIdInput, setPoIdInput] = useState(() => (Number.isFinite(initialPoId) ? String(initialPoId) : ''));
    const [selectedPoId, setSelectedPoId] = useState<number | null>(Number.isFinite(initialPoId) ? initialPoId : null);
    ...
    const [initializedPoId, setInitializedPoId] = useState<number | null>(null);
    ...
    useEffect(() => {
        if (!purchaseOrder || initializedPoId === purchaseOrder.id) {
            return;
        }

        setInitializedPoId(purchaseOrder.id);
        ...
        setLineItems(
            purchaseOrder.lines.map((line) => ({
                ...
                quantity: Math.max(1, Math.ceil(line.remainingQuantity ?? line.quantity ?? 1)),
                unitPrice: line.unitPrice ?? 0,
                uom: line.uom,
                taxCodeIds: [],
                include: true,
                remainingQuantity: line.remainingQuantity ?? Math.max(0, (line.quantity ?? 0) - (line.invoicedQuantity ?? 0)),
            })),
        );
        setDocumentFile(null);
        setSubmissionNote('');
    }, [initializedPoId, purchaseOrder]);

    const selectedLines = useMemo(() => lineItems.filter((line) => line.include && line.quantity > 0 && line.unitPrice > 0), [lineItems]);
    ...
    const readyForDraft = Boolean(
        purchaseOrder && invoiceNumber.trim().length > 0 && invoiceDate && selectedLines.length > 0,
    );
    ...
    const handleLoadPurchaseOrder = () => {
        ...
        setInitializedPoId(null);
    };

    const updateLine = (poLineId: number, patch: Partial<InvoiceLineFormState>) => {
        setLineItems((current) =>
            current.map((line) => (line.poLineId === poLineId ? { ...line, ...patch } : line)),
        );
    };
    ...
    const handlePersist = (mode: 'draft' | 'submit') => {
        ...
        const payload: SupplierInvoiceLineInput[] = selectedLines.map((line) => ({
                ...
                onSuccess: (invoice) => {
                    if (mode === 'submit') {
                        submitMutation.mutate(
                            { invoiceId: invoice.id, note: submissionNote.trim() || undefined },
                            {
                                onSuccess: (submitted) => {
        ...
        <div className="flex flex-1 flex-col gap-6">
            ...
            <div className="flex flex-wrap items-start justify-between gap-4">
                <div>
                    ...
                    <h1 className="text-3xl font-semibold text-foreground">Create invoice</h1>
                    <p className="text-sm text-muted-foreground">
                        Build an invoice from a purchase order, add taxes, and submit it for buyer review.
                    </p>
                </div>
                    ...
                    <div className="space-y-2">
                        ...
                        <div className="flex gap-2">
                            <Input
                                id="po-id"
                                value={poIdInput}
                                onChange={(event) => setPoIdInput(event.target.value)}
                                placeholder="Enter PO numeric ID"
                                inputMode="numeric"
                                disabled={isSaving}
                            />
                            <Button type="button" onClick={handleLoadPurchaseOrder} disabled={isSaving}>
                                Load
                            </Button>
                        </div>
                        <p className="text-xs text-muted-foreground">
                            Open a PO from the supplier dashboard, or paste the PO ID shared by your buyer.
                        </p>
                    </div>
                    {purchaseOrder ? (
                        <div className="rounded-lg border border-success/30 bg-success/5 p-4 text-sm">
                            <p className="font-semibold text-success">Loaded {purchaseOrder.poNumber}</p>
                            <p className="text-muted-foreground">{purchaseOrder.supplierName}</p>
                            <p className="text-xs text-muted-foreground">
                                {purchaseOrder.lines.length} lines · {formatCurrencyValue(purchaseOrder.totalMinor, purchaseOrder.currency)}
                            </p>
                        </div>
                    ) : null}
                </CardContent>
            ...
            {selectedPoId ? (
                <PurchaseOrderStateCard
                    ...
                    poQueryStatus={{ isLoading: poQuery.isLoading, isError: poQuery.isError, refetch: poQuery.refetch }}
                    formatCurrency={(value, currencyCode) => formatCurrencyValue(value, currencyCode)}
                />
            ) : null}
...
export interface PurchaseOrderStateCardProps {
    ...
    poQueryStatus: { isLoading: boolean; isError: boolean; refetch: () => unknown };
    formatCurrency: (value?: number | null, currency?: string | null) => string;
}

export function PurchaseOrderStateCard({ purchaseOrder, poQueryStatus, formatCurrency }: PurchaseOrderStateCardProps) {
    if (poQueryStatus.isLoading) {
        return (
            <Card className="border-border/70">
                <CardContent className="flex items-center gap-3 py-6 text-sm text-muted-foreground">
                    <Loader2 className="h-4 w-4 animate-spin" /> Loading purchase order details…
                </CardContent>
            </Card>
        );
    }
        ...
        return (
            <Card className="border-destructive/30 bg-destructive/5">
                <CardContent className="flex items-center justify-between gap-3 py-4">
                    <div>
                        <p className="font-semibold text-destructive">Unable to load purchase order.</p>
                        <p className="text-sm text-destructive/80">Ensure the PO exists and that you have access.</p>
                    </div>
                    <Button variant="outline" onClick={() => poQueryStatus.refetch()}>
                        Retry
                    </Button>
                </CardContent>
            </Card>
    ...
    return (
        <Card className="border-border/70">
            ...
            <CardContent className="grid gap-4 md:grid-cols-3">
                ...
                <SummaryItem label="Lines" value={`${purchaseOrder.lines.length} items`} />
                <SummaryItem label="Total value" value={formatCurrency(purchaseOrder.totalMinor, purchaseOrder.currency)} />
                <SummaryItem label="Revision" value={`Rev ${purchaseOrder.revisionNo}`} />
            </CardContent>
        </Card>
    );
}
        ...
        <Card className="border-border/70">
            ...
            <CardContent className="grid gap-4 md:grid-cols-4">
                ...
                <Field label="Due date">
                    <Input type="date" value={dueDate} onChange={(event) => onDueDateChange(event.target.value)} disabled={disabled} />
                </Field>
                <Field label="Currency">
                    ...
                    {poCurrency && poCurrency !== currency ? (
                        <p className="text-xs text-amber-600">PO currency is {poCurrency}; confirm if different.</p>
                    ) : null}
                </Field>
            </CardContent>
        </Card>
...
interface InvoiceLinesCardProps {
    ...
    onLineChange: (poLineId: number, patch: Partial<InvoiceLineFormState>) => void;
    currency: string;
    formatNumber: (value?: number, options?: Intl.NumberFormatOptions) => string;
    taxCodes: Array<{ id?: number | null; label: string; detail: string | null }>;
                ...
                {lineItems.length === 0 ? (
                    ...
                    <div className="space-y-4">
                        {lineItems.map((line) => {
                            ...
                            return (
                                <div key={line.poLineId} className="rounded-xl border border-border/60 p-4">
                                        ...
                                        {mode === 'create' ? (
                                            <Checkbox
                                                ...
                                                onCheckedChange={(checked) =>
                                                    onLineChange(line.poLineId, { include: Boolean(checked) })
                                                }
                                                disabled={disabled}
                                            />
                                        ) : (
                                            <span className="rounded-full bg-muted px-2 py-0.5 text-[11px] uppercase tracking-wide text-muted-foreground">
                                                Line {line.lineNo ?? line.poLineId ?? line.invoiceLineId ?? '—'}
                                            </span>
                                        )}
                                        <div className="flex flex-col">
                                            <p className="text-sm font-semibold text-foreground">
                                                Line {line.lineNo ?? line.poLineId}: {line.description}
                                            </p>
                                            <p className="text-xs text-muted-foreground">
                                                {maxQty ? `${formatNumber(maxQty)} remaining` : 'Qty per PO unknown'}
                                            </p>
                                        </div>
                                    </div>
                                    <div className="mt-4 grid gap-4 md:grid-cols-4">
                                        <Field label="Quantity">
                                            <Input
                                                ...
                                                min={1}
                                                value={line.quantity}
                                                onChange={(event) => onLineChange(line.poLineId, { quantity: Number(event.target.value) })}
                                        ...
                                        <Field label={`Unit price (${currency})`}>
                                            <Input
                                                ...
                                                onChange={(event) => onLineChange(line.poLineId, { unitPrice: Number(event.target.value) })}
                                                disabled={disabled || !line.include}
                                            />
                                        </Field>
                                        <Field label="UOM">
                                            <Input
                                                value={line.uom ?? ''}
                                                onChange={(event) => onLineChange(line.poLineId, { uom: event.target.value })}
                                                disabled={disabled || !line.include}
                                            />
                                        </Field>
                                        <div>
                                            <Label className="text-xs uppercase tracking-wide text-muted-foreground">Line total</Label>
                                            <p className="text-lg font-semibold text-foreground">
                                                {formatNumber(line.quantity * line.unitPrice, { style: 'currency', currency })}
                                            </p>
                                    ...
                                    <div className="mt-4 grid gap-4 md:grid-cols-2">
                                        <Field label="Description override">
                                            <Textarea
                                                ...
                                                rows={2}
                                                onChange={(event) => onLineChange(line.poLineId, { description: event.target.value })}
                                                disabled={disabled || !line.include}
                                            />
                                        </Field>
                                        <Field label="Tax codes">
                                            {allowTaxCodes ? (
                                                <TaxCodeMultiSelect
                                                    ...
                                                    options={taxCodes}
                                                    onChange={(next) => onLineChange(line.poLineId, { taxCodeIds: next })}
                                                    disabled={disabled || !line.include}
                                                />
                                            ) : (
                                                <p className="text-xs text-muted-foreground">
                                                    Tax codes are assigned by your buyer after submission.
                                                </p>
                                            )}
                                        </Field>
                                    </div>
...
function AttachmentsCard({ documentFile, onFileChange, disabled }: AttachmentsCardProps) {
    return (
        <Card className="border-border/70">
            <CardHeader>
            ...
            <CardContent className="space-y-3">
                <FileDropzone
                    label={documentFile ? documentFile.name : 'Upload signed invoice PDF'}
                ...
                {documentFile ? (
                    <Badge variant="outline" className="gap-2">
                        <BadgeDollarSign className="h-3 w-3" /> {documentFile.name}
                    </Badge>
                ) : (
                    <p className="text-xs text-muted-foreground">Attach your official invoice to speed up approval.</p>
                )}
            </CardContent>
        </Card>
    );
        ...
        <Card className="border-border/70">
            ...
            <CardContent className="space-y-4">
                <div className="rounded-lg border border-border/70 bg-muted/40 p-4">
                    <p className="text-sm text-muted-foreground">Total (ex tax)</p>
                    <p className="text-2xl font-semibold text-foreground">{totals.subtotal.toFixed(2)}</p>
                </div>
                <Field label="Buyer note (optional)">
                    <Textarea
                        ...
                        onChange={(event) => onSubmissionNoteChange(event.target.value)}
                        disabled={isSaving}
                    />
                </Field>
                <div className="flex flex-wrap gap-3">
                    <Button type="button" variant="outline" onClick={onSaveDraft} disabled={disabledDraft}>
                        {draftLabel ?? 'Save draft'}
                    </Button>
                    <Button type="button" onClick={onSubmit} disabled={disabledSubmit}>
                        {isSaving ? submitPendingLabel ?? 'Submitting…' : submitLabel ?? 'Submit invoice'}
                    </Button>
                </div>
                <p className="text-xs text-muted-foreground">
                    Drafts stay private to your team. Submitted invoices notify the buyer immediately.
                </p>
            </CardContent>
        </Card>
    );
...
interface TaxCodeMultiSelectProps {
    value: number[];
    options: Array<{ id?: number | null; label: string; detail: string | null }>;
    onChange: (next: number[]) => void;
    disabled?: boolean;
}

function TaxCodeMultiSelect({ value, options, onChange, disabled }: TaxCodeMultiSelectProps) {
    if (options.length === 0) {
        return <p className="text-xs text-muted-foreground">No tax codes configured.</p>;
    }
    ...
    return (
        <div className="space-y-2">
            <div className="flex flex-wrap gap-2">
                {options.map((option) => {
                    const active = option.id ? value.includes(option.id) : false;
                    return (
                        <button
                            ...
                            className={cn(
                                ...
                                active ? 'border-primary bg-primary/10 text-primary' : 'border-border/70 text-muted-foreground',
                            )}
                            onClick={() => handleToggle(option.id)}
                            disabled={disabled}
                        >
                            <span>{option.label}</span>
                            {option.detail ? <span className="ml-1 text-[10px]">{option.detail}</span> : null}
                        </button>
                    );
                })}
            </div>
            {value.length === 0 ? (
                <p className="text-xs text-muted-foreground">Select applicable taxes or leave blank for tax-exempt.</p>
            ) : null}
        </div>
    );
}
