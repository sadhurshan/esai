...
export function NotificationStreamProvider({ children }: NotificationStreamProviderProps) {
    ...
    useEffect(() => {
        ...
        const channel = echo.private(channelName).listen(NOTIFICATION_EVENT, () => {
        ...
        return () => {
            ...
            const connector = echo.connector as {
                pusher?: { connection?: { state?: string; socket?: { readyState?: number } } };
            };
            const connection = connector.pusher?.connection;
            const connectionState = connection?.state;
            const socketReadyState = connection?.socket?.readyState;
            const isSocketClosing = socketReadyState === 2 || socketReadyState === 3; // WebSocket CLOSING/CLOSED
